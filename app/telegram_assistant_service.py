...
def _relative_ago_clock_from_text(text: str) -> str:
    """Relative minutes/hours from now, shared with the deterministic parser."""
    from app.assistant_rule_fallback import _relative_ago_minutes
    ...
    when = datetime.now(ZoneInfo("Europe/Athens")) - timedelta(minutes=ago_minutes)
...
def _inherit_conversation_context(
    ...
    *,
    ...
    if not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", time_value):
        time_value = _relative_ago_clock_from_text(user_text) or _clock_from_text(user_text)
