...
def test_contiguous_base_and_extras_remain_unchanged():
    day = build_timekeeping_report([_row(
        declared="09:00–17:00", proposed="09:00–17:00",
        actual="09:00–19:00", status="change", rule_id="OVERTIME_ONLY",
        declared_minutes=480, contract_kind="Πλήρης", weekly_days=5,
        daily_overtime_basis_minutes=480, overwork_minutes=60,
        overtime_minutes=60,
        overtime_segments=[{
            "date": "17/08/2026", "from": "18:00", "to": "19:00", "minutes": 60,
    ...
    assert day["basis_label"] == "09:00–17:00"
    assert day["recognized_base_segments"] == ["09:00–17:00"]
    assert day["recognized_basis_rule"] == "same_start_declared_plus_break"
    assert not any("ευθυγραμμίστηκε χρονικά" in item for item in day["warnings"])


def test_short_proposal_base_is_reconciled_only_when_extras_are_discontinuous():
    day = build_timekeeping_report([_row(
        declared="13:00–17:00", proposed="13:00–17:00",
        actual="13:00–00:10", status="change", rule_id="OVERTIME_ONLY",
        declared_minutes=240, contract_kind="Πλήρης", weekly_days=5,
        daily_overtime_basis_minutes=480, overwork_minutes=60,
        overtime_minutes=130,
        overtime_segments=[{
            "date": "17/08/2026", "from": "22:00", "to": "00:10", "minutes": 130,
        }],
    )])["days"][0]

    assert day["basis_label"] == "13:00–21:00"
    assert day["recognized_base_segments"] == ["13:00–21:00"]
    assert day["recognized_work_minutes"] == 480
    assert day["overwork_minutes"] == 60
    assert day["overtime_minutes"] == 130
    assert day["recognized_basis_rule"].endswith("+extra_timeline_reconciled")


def test_missing_entry_backward_base_uses_same_boundary_as_extras():
    day = build_timekeeping_report([_row(
        declared="08:00–16:00", proposed="09:15–17:15",
        actual="08:00–17:15", status="change",
        rule_id="MISSING_ENTRY_EXTRA_BACKWARD", punch_completeness="Τεκμαρτό",
        declared_minutes=480, contract_kind="Πλήρης", weekly_days=5,
        daily_overtime_basis_minutes=480, overwork_minutes=60,
        overtime_minutes=15,
        overtime_segments=[{
            "date": "17/08/2026", "from": "17:00", "to": "17:15", "minutes": 15,
        }],
    )])["days"][0]

    assert day["basis_label"] == "08:00–16:00"
    assert day["recognized_base_segments"] == ["08:00–16:00"]
    assert day["recognized_work_minutes"] == 480
    assert day["overwork_minutes"] == 60
...
def test_unsafe_extra_timeline_discontinuity_requires_review():
    row = _row(
        ...
        declared_minutes=400, contract_kind="Πλήρης", weekly_days=6,
        daily_overtime_basis_minutes=400, overwork_minutes=80,
        overtime_minutes=240,
        overtime_segments=[{
            "date": "17/08/2026", "from": "21:24", "to": "01:24", "minutes": 240,
        }],
    )
