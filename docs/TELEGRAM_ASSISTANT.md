...
αποθηκεύονται ως `awaiting_pin`, ενώ οι ασαφείς ως
...
2. ο χρήστης καλείται να στείλει απευθείας τον προσωπικό 4ψήφιο PIN αν συμφωνεί,
3. ο σωστός PIN αλλάζει την κατάσταση σε `confirmed_dry_run` και εμφανίζει τι
   **ΘΑ** εκτελεστεί όταν ενεργοποιηθεί η υπηρεσία.

Δεν υπάρχει ενδιάμεσο βήμα `ΝΑΙ/ΟΧΙ`. Μόνο ένα τετραψήφιο μήνυμα συσχετίζεται
deterministic με το σχετικό bot reply ή με το πιο πρόσφατο pending task του
ίδιου εξουσιοδοτημένου chat. Οποιοδήποτε άλλο κείμενο αναλύεται ως νέα εντολή.
Η επαλήθευση PIN δεν ξαναστέλνεται στο Gemini.
...
- `karta_assistant_task`: δομημένη πρόθεση, validation, περιγραφή ενέργειας και
  κατάσταση συνομιλίας (`awaiting_pin`, `confirmed_dry_run`, `pin_locked` ή
  `needs_clarification`).
