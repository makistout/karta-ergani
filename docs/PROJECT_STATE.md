...
- `/ui/stores/notify`: λήπτες Telegram/Email· ενέργειες (auto-close, καθυστέρηση
  ...
  επιλογή ενεργού· στις Ρυθμίσεις μόνο τις Αργίες (`settings.holidays.*`)· όχι
  ειδοποιήσεις/ενέργειες/listener/απολογιστικό ρυθμίσεων.
...
- Ρόλοι στο `app/access_control.py` (`ROLE_PERMISSIONS`). Νέος ρόλος **`accountant`**:
  ...
  μόνο επεξεργασία αργιών. Με `schedule.submit_daily` υποβάλλει απολογιστικές
  μεταβολές/υπερωρίες στο Ergani (μεμονωμένα και μαζικά). Χωρίς `stores.manage` /
  credentials — μόνο επιλογή ενεργού καταστήματος.
