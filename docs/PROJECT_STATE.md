...
- `/ui/stores/notify`: λήπτες Telegram/Email· ενέργειες (auto-close, καθυστέρηση
  ...
  έγκριση όλων των μεταβολών. Ρόλος **`accountant`**: βλέπει μόνο τις Αργίες
  (`settings.holidays.*`)· όχι ειδοποιήσεις/ενέργειες/listener/απολογιστικό.
...
- Ρόλοι στο `app/access_control.py` (`ROLE_PERMISSIONS`). Νέος ρόλος **`accountant`**:
  ...
  εργαζόμενοι) χωρίς Συγχρονισμό / Καταστήματα / Καταγραφές / Χρήστες· στις Ρυθμίσεις
  μόνο επεξεργασία αργιών. Με `schedule.submit_daily` υποβάλλει απολογιστικές
  μεταβολές/υπερωρίες στο Ergani (μεμονωμένα και μαζικά).
