...
- **Πρωτόκολλα Ergani** (`protocol_from` / `protocol_to` στο `karta_work_log`): συμπληρώνονται
  αυτόματα από 1-1 απαγωγή με `karta_ergani_protocol` (ώρα πραγματικής ↔ `submit_at`).
  Εμφανίζονται διακριτικά κάτω από **Από/Έως**. Αν στην ίδια στιγμή υπάρχουν πολλαπλά
  πρωτόκολλα ή πολλαπλές ώρες, το πεδίο μένει κενό (δεν μαντεύουμε).
