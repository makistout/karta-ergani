...
scanner **ρωτά για επιβεβαίωση** και, αν εγκριθεί, ξαναστέλνει ως διόρθωση
(`correction_mode`). Τα χτυπήματα από scanner σημειώνονται με κανάλι/πηγή
**Scanner** και εμφανίζονται στο tab **Scanner** των καταγραφών
(`/ui/sync-log#scanner`).
Λεπτομέρειες: [SCANNER_PWA.md](SCANNER_PWA.md).

Η ωρομέτρηση χρησιμοποιεί την έκδοση `timekeeping-v18-rest-first-sunday-fallback`. Στην
πλήρη ανισομερή κατανομή η βάση, η επέκταση ανισομερούς και η υπερεργασία είναι
αποκλειστικές κατηγορίες, με εβδομαδιαία κατανομή Δευτέρα–Κυριακή έως τις 40 ώρες.
Ενιαίο δηλωμένο ή εγκεκριμένο προτεινόμενο 9ωρο/10ωρο τεμαχίζεται επίσης στις
ίδιες κατηγορίες, ακόμη και χωρίς χτύπημα, χωρίς διπλομέτρηση ήδη χαρακτηρισμένων
λεπτών του Απολογιστικού.
Όταν υπάρχει μικρότερη πλήρης κάρτα, οι ζώνες του δηλωμένου μεγάλου ωραρίου
διατηρούνται· μεγαλύτερη καθαρή διάρκεια κάρτας μπορεί να τις επεκτείνει.
Η συγχωνευμένη μηνιαία αναφορά δηλώνει αντίστοιχα έκδοση
`timekeeping-v18-rest-first-sunday-fallback-month`.
...
- Αν το sync πραγματικής γυρίσει **αβέβαιο κενό** (`empty_uncertain`: Excel χωρίς αρχείο,
  χωρίς HTML), **δεν** αντικαθίσταται η βάση με κενό και το αυτόματο `late_check_in`
  παραλείπεται (`work_log_empty_uncertain`) — αποφεύγει ψευδείς καθυστερήσεις όταν το
  portal καθυστερεί να εμφανίσει χτυπήματα.
