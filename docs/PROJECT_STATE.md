...
- Στο `/ui/schedule`:
  - **Κατέβασμα Excel** (τρέχουσα / επόμενη εβδομάδα) → `.xlsx` με όλους τους εργαζόμενους
    και **προγεμισμένες ώρες/ΡΕΠΟ** από το ψηφιακό ωράριο Ergani (`karta_schedule`).
...
- `/ui/users`: λίστα με ενέργειες επαναποστολής email, **καταγραφών** (`GET …/activity`) και
  **διαγραφής** (`DELETE /api/users/<id>`, δικαίωμα `users.delete`).
...
|---------------|---------------------|
| `late_check_in` | Ψηφ. ωράριο εργασίας, χωρίς είσοδο που καλύπτει το διάστημα, ≥ grace από **αρχή ωραρίου** (15/30/45′). Πρόωρη άφιξη (π.χ. 07:24 για 07:30) **δεν** ενεργοποιεί. Μία φορά/ημέρα (auto). |
