...
- Αν ο Listener λάβει οριστικό HTTP 400 από το ΕΡΓΑΝΗ επειδή απαιτείται λόγος
  καθυστέρησης και το αρχικό payload δεν είχε αιτιολογία, το erganiOS δημιουργεί
  **ένα μόνο νέο idempotent job** με `f_aitiologia=001`. Η δεύτερη υποβολή γίνεται
  ξανά από τον ίδιο Listener· δεν χρησιμοποιείται direct fallback. Δεν εκτελείται
  τρίτη προσπάθεια αν το ΕΡΓΑΝΗ απορρίψει και το διορθωμένο payload.
...
- Στις **Καταγραφές → Χτυπήματα κάρτας** αποθηκεύονται το HTTP status, το
  `error_summary` και η δομημένη απάντηση του ΕΡΓΑΝΗ που επέστρεψε ο Listener.
  Έτσι η στήλη «Λεπτομέρειες» εμφανίζει την πραγματική αιτία απόρριψης αντί για
  το γενικό μήνυμα «HTTP 400 — δεν επέστρεψε αναλυτικό μήνυμα».
