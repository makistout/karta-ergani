...
Η Ergani απαιτεί ή απαγορεύει τον κωδικό καθυστέρησης ανάλογα με το αν η υποβολή έγινε εντός 15 λεπτών από την ώρα κίνησης. Η εφαρμογή **δεν** αφήνει το frontend να στέλνει πάντα `001` ούτε συνδέει την αιτιολογία με το ψηφιακό ωράριο ή την ευέλικτη προσέλευση.

Κανόνας:

- `submitted_at - event_at <= 15 λεπτά` → χωρίς αιτιολογία
- `submitted_at - event_at > 15 λεπτά` → `001`
...
Η σύνδεση με την πραγματική:

1. **Δικά μας χτυπήματα** (`karta_card_event` + `karta_declaration.protocol`): γράφονται πάντα
   στο αντίστοιχο `protocol_from` / `protocol_to` (κενό πεδίο μόνο).
2. **1-1 στα υπόλοιπα**: κλειδί `(store_id, ημερολογιακή ημέρα, HH:MM)` από `submit_at`
   ↔ κενό `hour_from` / `hour_to`. Τα ήδη γεμάτα πεδία και τα πρωτόκολλά τους εξαιρούνται.
   Έτσι δύο χτυπήματα την ίδια ώρα, εκ των οποίων ένα δικό μας → το δικό μας έχει πρωτόκολλο,
   το άλλο παίρνει το εναπομείναν. Αν μείνουν >1 άγνωστα, μένουν κενά.
...
2. Επιβεβαίωση χρήστη → **WTOWeek ανά εργαζόμενο** (ένα πρωτόκολλο για τις 7 ημέρες,
   όχι WTODaily ανά ημέρα). Οι κενές ημέρες στο Excel συμπληρώνονται από το τρέχον
   πρόγραμμα. Υποβάλλονται και επιβεβαιώσεις (`same`) όταν έχουν συμπληρωθεί ώρες/ΡΕΠΟ.
   Στο payload WTOWeek: ΡΕΠΟ→`ΑΝ`, ώρες→`ΕΡΓ`, χωρίς ωράριο και χωρίς ΡΕΠΟ→`ΜΕ`.
...
ημερομηνίες· χωρίς προγεμισμένες ώρες/ΡΕΠΟ από `karta_schedule`). Η στήλη **Ώρες**
είναι τύπος που μετατρέπει και ακεραίους HHMM (`1100`) σε χρόνο μέσω `TIME()`,
ώστε το `[h]:mm` να μην εμφανίζει ημέρες ως ώρες (π.χ. `19200:00`). Το download
στέλνεται με `Cache-Control: no-store`.
...
## Διαγραφή καταστήματος (ΑΦΜ + παράρτημα)

Το `DELETE` στο `karta_store_config` δεν αρκεί: ωράριο, πραγματική, κάρτες, συμβάσεις και
πρωτόκολλα κλειδώνουν σε **ΑΦΜ εργοδότη + παράρτημα**, όχι μόνο `store_id`. Η διαγραφή
περνά από `app/store_purge.py` και καθαρίζει ΒΔ + `protocol_pdfs/{afm}/{branch}/` +
excel debug του store, εφόσον κανένα άλλο store δεν κρατά το ίδιο παράρτημα.
