...
## 2026-09-26 — Ειδοποίηση ορφανών χτυπημάτων στις 10:00

- Όπως οι υπερβάσεις στις 21:00, στις **10:00** στέλνεται digest στους λήπτες
  για ορφανά χτυπήματα της προηγούμενης ημέρας (είσοδος χωρίς έξοδο, έξοδος
  χωρίς είσοδο), μαζί με εξόδους μετά τα μεσάνυχτα που ανήκουν στη χθεσινή
  βάρδια. Ζευγάρι που ενώθηκε overnight δεν ειδοποιείται.
- Env: `KARTA_SCHEDULED_ORPHAN_PUNCH_NOTIFY_ENABLED`,
  `KARTA_SCHEDULED_ORPHAN_PUNCH_NOTIFY_TIME` (προεπιλογή `10:00`).
- Μία φορά ανά κατάστημα/ημέρα μέσω sync log
  (`scheduled_orphan_punch_notify`).
- Απάντηση «κλείσε / κλειστά» στο digest κλείνει τους αναφερόμενους στην
  ημερομηνία του μηνύματος με την ώρα λήξης ωραρίου — όχι «τώρα» με χθεσινή
  ημερομηνία αναφοράς (ο Ergani το απορρίπτει). Πολλοί μαζί: ο καθένας τη
  δική του λήξη + απόσταση **50–100″** στο χτύπημα (όχι ζωντανή αναμονή).

## 2026-09-26 — Σελίδα Παρουσιάσεις

- Υπομενού **Παρουσιάσεις** στις Τιμολογήσεις: μικρό πεδίο email και
  **Αποστολή** στην ίδια γραμμή, προεπισκόπηση των δύο PDF, έλεγχος μορφής
  και BCC `info@erganios.gr`.

## 2026-09-26 — Αποστολή παρουσίασης πελάτη

- Στην καρτέλα πελάτη, το **Αποστολή παρουσίασης** στέλνει στο καταχωρημένο
  email τα δύο PDF προτύπου (`Απολογιστικό-Ωρομέτρηση`, `AI-Agent`) με κείμενο
  παρουσίασης και στοιχεία επικοινωνίας (`BILLING_CONTACT_NAME`,
  `BILLING_CONTACT_PHONE`, `BILLING_ISSUER_EMAIL`, `PUBLIC_BASE_URL`).

## 2026-09-26 — Προσφορά πελάτη

- Στην καρτέλα πελάτη, δίπλα στο συμφωνητικό, κατεβαίνει συμπληρωμένη η
  εμπορική προσφορά (επωνυμία + σημερινή ημερομηνία). Χωρίς εκπρόσωπο δεν
  κατεβαίνει.

## 2026-09-26 — Τιμές δημόσιου τιμοκαταλόγου

- Landing: 1-2 **90€**, 3-5 **110€**, 6-20 **150€**, 21-50 **190€**,
  51-100 **275€**, 101-200 **440€** (ίδιες με τα πακέτα 12 μηνών).

## 2026-09-26 — Συμφωνητικό ανάληψης ευθύνης στον πελάτη

- Η καρτέλα πελάτη έχει πεδίο **Εκπρόσωπος** και διακόπτη **Ενεργός**.
- Διακόπτης **Συμφωνητικό** συμπληρώνει το Word με στοιχεία εκδότη και πελάτη
  και το κατεβάζει.

## 2026-09-26 — Αναζήτηση ονόματος στην Αρχική

- Η Αρχική έχει το ίδιο πεδίο «Αναζήτηση ονόματος» με το απολογιστικό
  (χωρίς τόνους) και φιλτράρει τον πίνακα χωρίς νέα κλήση.

## 2026-09-25 — Νέα εντολή AI Agent ρωτάει κατάστημα

- Αν ο λήπτης έχει πολλά καταστήματα, νέα εντολή (όχι reply, όχι συνέχεια
  διευκρίνισης) ρωτάει πρώτα «Επιλέξτε κατάστημα». Δεν κληρονομεί κατάστημα
  από την τελευταία ειδοποίηση.
- Κατάστημα μπαίνει αυτόματα μόνο με ένα επιτρεπόμενο, ρητό όνομα στο μήνυμα
  ή reply σε συγκεκριμένο μήνυμα.
- Named punch: ΑΦΜ από το LLM μόνο αν ταιριάζουν με τα ονόματα του μηνύματος.

## 2026-09-25 — Χρήστες: διακόπτης και πλήθος καταστημάτων

- Το «Ενεργός» στη φόρμα χρήστη είναι διακόπτης.
- Στη λίστα φαίνεται σε πόσα καταστήματα έχει πρόσβαση ο καθένας
  (`3 καταστήματα` / `Όλα (N)` για super_admin).
- Η δημιουργία χρήστη γράφει αυτόματα στον κατάλογο όσα δικαιώματα λείπουν
  (`billing.manage`), ώστε να μην σπάει το FK στο `karta_permission`.

## 2026-09-25 — Αληθοφανή δευτερόλεπτα στα μαζικά χτυπήματα

- Η απόσταση ανάμεσα σε χτυπήματα παρτίδας είναι τυχαία **50–100″**, με
  πραγματικά δευτερόλεπτα (`12:17:53` / `12:19:12`), όχι `:00` και όχι 1–2 λεπτά.
- Στο «πριν» φεύγουν αμέσως· στο «τώρα» περιμένει το ίδιο διάστημα πριν την
  επόμενη υποβολή.

## 2026-09-24 — Στιγμιότυπο προσωπικού

- 15 καταστήματα: 413 ενεργοί εργαζόμενοι, 403 μοναδικά ΑΦΜ
  (`PROJECT_STATE.md`).

## 2026-09-24 — AI Agent: πρόοδος ουράς και δεύτερο άνοιγμα

- Μαζικά χτυπήματα «τώρα» μένουν με απόσταση 1–2 λεπτά. Telegram/UI λένε
  `Εκτελέστηκε:` / `Περιμένει:` ώστε να φαίνεται τι έγινε και τι ακολουθεί.
- «Άνοιξε όλους»: κόβει μόνο όσους έχουν **ακόμα ανοιχτή** κάρτα. Όσοι
  άνοιξαν και έκλεισαν μπορούν να ανοίξουν ξανά.

## 2026-09-24 — Τιμολόγηση πελατών (αρχικό module)

- Νέοι πίνακες πελατών, σύνδεσης καταστημάτων, πακέτων συνδρομής
  και παραστατικών ΑΠΥ/ΤΠΥ. Δεν αλλάζει την υπάρχουσα λειτουργία.
- Μενού **Τιμολογήσεις** με Πελάτες, Συνδρομές, Τιμολόγια.
  Έκδοση: επιλογή πελάτη + συνδρομών και αποστολή στο Oxygen sandbox
  (ίδια ροή με room spot=0). Έντυπο Α4, πιστωτικό Oxygen, αριθμός
  πιστωτικού χωρίς link όταν έχει ήδη εκδοθεί.
- Οδηγός: `docs/BILLING.md`.

## 2026-09-24 — Scanner, κινητά καταστήματα και ένδειξη AI Agent

- **Scanner login:** το όνομα χρήστη συγκρίνεται χωρίς πεζά/κεφαλαία (IKA/EFKA
  από κινητό). Το portal login χρησιμοποιεί το αποθηκευμένο username. Το
  `Origin` δέχεται και `www` και χωρίς `www` για το `PUBLIC_BASE_URL`.
- **Scanner ώρα / offline:** λάθος ζώνη στο κινητό δεν απορρίπτει την κάρτα αν
  το ISO είναι έως 24 ώρες στο μέλλον (γίνεται «τώρα»). Offline ουρά στέλνει
  ώρα Αθήνας και εκπρόθεσμη υποβολή παίρνει αιτιολογία `003`. Στην «Αποστολή
  εκκρεμών» φαίνεται spinner όσο διαρκεί η διαδικασία.
- **Καταστήματα (κινητό):** τα κουμπιά ενεργειών δεν κόβονται· εμφανίζονται σε
  πλέγμα 2×2 με ετικέτα (Επιλογή, Επεξεργασία, Ρυθμίσεις, Διαγραφή).
- **Sidebar:** η ένδειξη υγείας γράφει `AI Agent` (πράσινο / προσοχή / σφάλμα),
  όχι «Telegram OK».

## 2026-09-24 — Έκδοση διαγραμμάτων για σημείωση στήλης Πρόταση

- Νέα ελεγμένη έκδοση `rules-98cc95cc79e8297a`: το `APOLOGISTIKO_LOGIC.md`
  είχε ήδη τη σημείωση εμφάνισης σπαστής πρότασης / Χ, χωρίς αλλαγή κανόνων.
- Το `--check` ξανασυμφωνεί με τις πηγές, ώστε το workflow Rule diagrams
  να μην αποτυγχάνει σε κάθε επόμενο commit.

## 2026-09-23 — Ένδειξη υγείας Telegram assistant για super admin

- Τακτικός έλεγχος ότι το `telegram_assistant_service.py` φορτώνει (σύνταξη)
  και ότι δεν έσκασε εντολή μετά την τελευταία επιτυχημένη (`completed`/`answered`).
- Ο super admin βλέπει ένδειξη κάτω από το λογότυπο (OK / προσοχή / σφάλμα).
  AJAX κάθε λεπτό, cache 15s· παλιό crash μέσα στις 6 ώρες δεν μένει κίτρινο.
- `GET /api/assistant/health` μόνο για super admin. Η σελίδα γεμίζει την ένδειξη
  με AJAX, όχι από το template, ώστε σφάλμα στον έλεγχο να μην ρίχνει το office.

## 2026-09-23 — Πρόταση απολογιστικού: όλα τα ωράρια και αφαίρεση σπαστού

- Η στήλη Πρόταση δείχνει όλα τα διαστήματα. Σε σπαστό το δεύτερο ζεύγος
  μπαίνει από κάτω, όπως στα χτυπήματα, χωρίς αποκοπή με αποσιωπητικά.
- Στο modal αλλαγής πρότασης το δεύτερο Από/Έως έχει Χ: αφαιρείται και μένει
  ενιαίο ωράριο. Επανέρχεται με «Δεύτερο τμήμα». Οι κανόνες υπολογισμού δεν
  αλλάζουν.

## 2026-09-23 — Τεκμηρίωση αρχικής κάρτας πριν από επέκταση πλήρους βάσης

- Η επέκταση μικρής δήλωσης πλήρους απαιτεί χρονική κάλυψη από την ιστορική
  σύμβαση και μοναδικό αρχικό πλήρες ζεύγος που καλύπτει την καθαρή ημερήσια βάση.
- Νέος `FULL_SHORT_EVIDENCE_REVIEW`: πολλαπλά/ελλιπή/συνενωμένα στοιχεία ή
  ανεπαρκής ιστορική κάλυψη οδηγούν σε Έλεγχο, με παλιά πρόταση μόνο ως αναφορά.
- Τοπικός έλεγχος σε δεδομένα 22/09: 41 επεκτάσεις και 28 έλεγχοι στις 69
  γνωστές εγγραφές. Καμία επίδραση στις υπόλοιπες 31.199. Δεν αλλάζει η ωρομέτρηση.
- Νέα έκδοση snapshot, τεκμηρίωσης και ιδιωτικών διαγραμμάτων. Ο έλεγχος
  ιστορικών αρχείων δέχεται μόνο ισοδύναμες LF/CRLF εκδοχές κειμένου, χωρίς
  αλλαγή παλιών αρχείων, hashes ή ελέγχου δυαδικών αρχείων.

## 2026-09-22 — Τοπική διόρθωση μικρής δήλωσης πλήρους απασχόλησης

- Νέος κανόνας `FULL_SHORT_DECLARATION_BASE` πριν από την ευελιξία:
  πλήρης συνεχόμενη εργασία που καλύπτει τη συμβατική βάση προτείνεται με
  αυτή τη διάρκεια, ακόμη κι αν το αρχικό δηλωμένο είναι μικρότερο.
- Διατηρούνται αρχική δήλωση, διαφορές και πρόσθετες ώρες. Εξαιρούνται
  ελλιπή/τεκμαρτά, ορφανά, σπαστό, POST_CARRY, ειδική κατανομή και διαφωνία
  συμβατικής/ημερήσιας βάσης. Νέα έκδοση υπολογισμού απολογιστικού.
- Η αναγνωρισμένη βάση ωρομέτρησης παραμένει στον δικό της υπολογισμό.
  Η αλλαγή αφορά την πρόταση απολογιστικού, χωρίς εγγραφή στη βάση ή υποβολή.
- Ενημέρωση καταλόγου, λογικής και διαγραμμάτων με νέα ιδιωτική έκδοση.

## 2026-09-23 — Rule diagrams CI: hash ανεξάρτητα από CRLF

- Το `--check` συγκρίνει παραδοτέα και με LF και με CRLF, ώστε το GitHub
  Actions (Linux) να μην αποτυγχάνει όταν τα hashes γράφτηκαν στα Windows.
- Νέα έκδοση διαγραμμάτων `rules-8ebf3a077ddf80ee`.

## 2026-09-23 — Καμπάνα: και χθεσινές νυχτερινές έξοδοι

- Το post-sync κοιτάει πλέον σήμερα **και χθες**. Ανοιχτή βάρδια που λήγει
  μετά τα μεσάνυχτα (π.χ. 17:00–01:00) παίρνει `late_check_out` την επόμενη
  μέρα, αντί να χάνεται επειδή βρισκόταν μόνο στη χθεσινή αναφορά.

## 2026-09-22 — Ψηφιακό ωράριο ξανά σε όλους τους ρόλους

- Το μενού, οι γρήγορες ενέργειες και το `GET /api/schedule/*` ξανανοίγουν με
  `schedule.view` για όλους τους ρόλους που έχουν προβολή ωραρίου.
- Πραγματική απασχόληση, Πρωτόκολλα, Απολογιστικό και προειδοποιήσεις σύμβασης
  παραμένουν μόνο σε λογιστή και super admin.

## 2026-09-22 — Χτύπημα από/έως = έναρξη/λήξη εργασίας

- Στον κατάλογο Excel τα κελιά «Χτύπημα από/έως» παίρνουν την πρώτη πλήρη
  είσοδο και την τελευταία πλήρη έξοδο. Ορφανή ημιτελής γραμμή δεν αδειάζει
  πλέον το «από». Δεν ταυτίζονται με το αναγνωρισμένο ωράριο.

## 2026-09-22 — Canonical αναγνωρισμένο/υπερεργασία

- Η ωρομέτρηση εκθέτει πλέον `recognized_interval`/`basis_label` και
  `overwork_interval`/`overwork_from`/`overwork_to` από τα ίδια canonical
  timelines που χρησιμοποιεί για τις προσαυξήσεις.
- Προστέθηκε έλεγχος `datetime` ότι η υπερεργασία αρχίζει ακριβώς στη λήξη του
  αναγνωρισμένου ωραρίου· ασυνέχεια καταγράφεται ως warning.
- Ο κατάλογος Excel χρησιμοποιεί αποκλειστικά τα canonical πεδία και δεν
  εμφανίζει πλέον το `proposed` ως αναγνωρισμένο.
- Ελλιπές χτύπημα (μόνο είσοδος ή μόνο έξοδος) δεν ταξινομεί πρόσθετο χρόνο από
  τεκμαρτό όριο. Η πρόταση/αναγνωρισμένη βάση εξακολουθεί να ανακατασκευάζεται.

## 2026-09-21 — Οθόνες συμμόρφωσης μόνο σε λογιστή & super admin

- Ψηφιακό ωράριο, Πραγματική απασχόληση, Πρωτόκολλα, Απολογιστικό και οι
  προειδοποιήσεις παραβάσεων σύμβασης στην Αρχική εμφανίζονται μόνο σε
  `accountant` και `super_admin`. Ο office manager (όπως και office, viewers,
  backoffice admins) δεν τα βλέπει σε μενού, γρήγορες ενέργειες ή συνδέσμους.
- Νέα permissions `schedule.page.view`, `work_log.page.view`, `protocols.view`,
  `apologistic.view`, `alerts.contract.view`. Ο έλεγχος είναι ρόλου, οπότε
  stale session/DB permissions δεν ξεκλειδώνουν τις οθόνες.
- Backend κλείδωμα: `/ui/schedule`, `/ui/work-log`, `/ui/protocols`,
  `/ui/apologistic*`, `GET /api/schedule/*`, `GET /api/protocols/list` και όλα
  τα `/api/apologistic/*` (μαζί με υποβολές και Excel ωρομέτρησης).
- Παραμένουν κοινά όσα χρειάζεται η Ψηφιακή κάρτα: `/api/work-log/list`,
  `/api/work-log/history`, `/ui/work-log/history`, PDF πρωτοκόλλου,
  `action-settings`. Η αναφορά αρχικής στέλνει `links` και το `<body>` έχει
  `data-compliance` για να μη δείχνονται σύνδεσμοι σε κλειδωμένες σελίδες.

---

## 2026-09-21 — `/mobile`: αναζήτηση σε όλο το προσωπικό

- Η αναζήτηση επιστρέφει τον εργαζόμενο ανεξάρτητα από το σημερινό πρόγραμμα:
  ενεργές συνδέσεις `karta_employment` και ωράρια τελευταίων 30 ημερών χωρίς
  ακόμη σύνδεση απασχόλησης.
- Νέα ενότητα «Εκτός σημερινού προγράμματος» με ένδειξη «Χωρίς ωράριο σήμερα»,
  ορατή μόνο με όρο αναζήτησης ή όσο παραμένουν επιλεγμένοι.
- Χωρίς ωράριο δεν υπάρχει μέτρο πληρότητας: δεν σημαίνονται ως ολοκληρωμένη
  ημέρα και παραμένουν επιλέξιμοι για άνοιγμα/κλείσιμο.
- Η υποβολή ελέγχει πλέον ότι ο εργαζόμενος ανήκει στο κατάστημα, όχι ότι έχει
  σημερινό ωράριο. Χωρίς αλλαγή σε κανόνες κάρτας/απολογιστικού.

---

## 2026-09-21 — Mobile portal εντολών

- Νέο `/mobile` με σύνδεση χρήστη γραφείου και δικαιώματα καταστήματος/κάρτας.
- Συμπαγή κουτάκια προσωπικού ανά ειδικότητα, σημερινό ωράριο και χτυπήματα.
- Ολοκληρωμένες ημέρες σε κοινή ανενεργή ενότητα στο τέλος.
- Πολλαπλή επιλογή, σταθερά Άνοιγμα/Κλείσιμο και Τώρα/Πριν 5′/Πριν 10′.
- Επαναχρησιμοποίηση της υποβολής κάρτας, server ώρα Ελλάδας, έλεγχοι stale
  καταστήματος/ημέρας, αποτελέσματα ανά εργαζόμενο και ανανέωση χτυπημάτων.
- Οδηγός `docs/MOBILE_PORTAL.md` και regression tests του mobile portal.

---

## 2026-09-20 — Κανόνες & διαγράμματα μόνο για super admin

- Νέα επιλογή κάτω από το Απολογιστικό με 86 λογικά διαγράμματα, αναζήτηση,
  εκτύπωση, σειρά εκτέλεσης, κατάλογο, αναλυτική λογική και αποκλίσεις.
- Προστασία password session + ρητού ρόλου super_admin σε σελίδα και κάθε αρχείο.
- Εμφανής έκδοση πηγών, εκδόσεις υπολογισμών, ιστορικό και πλήρες ZIP.
- Immutable αρχειοθέτηση, έλεγχος fingerprints, προειδοποίηση ασυμφωνίας,
  CI tests και υποχρεωτική διαδικασία ανανέωσης σε κάθε αλλαγή κανόνων.
- Το ιστορικό Word εργατολογικού ελέγχου επισημαίνεται ως έκδοση 14/08/2026 (v2).

---

## 2026-09-19 — Σπαστή βάρδια: δεύτερο χτύπημα + Telegram roster

- **Κάρτα / guards:** μετά κλειστό πρωινό ζεύγος (είσοδος+έξοδος) επιτρέπεται
  νέο άνοιγμα/κλείσιμο την ίδια μέρα. Μπλοκάρισμα μόνο όταν υπάρχει ανοιχτό
  ζεύγος (`card_has_open_cycle` / ανοιχτή πραγματική).
- **Υποβολή WRKCardSE:** το `409` δεν εμφανίζεται πλέον για κάθε ίδιο τύπο
  την ίδια μέρα· μόνο σε πραγματική σύγκρουση με το τρέχον ανοιχτό/κλειστό κύκλο.
- **UI κείμενο:** scanner / office / retro μιλούν για **νέο χτύπημα**, όχι
  «διόρθωση» (το εσωτερικό `correction_mode` παραμένει για το API).
- **Telegram AI:** ο κατάλογος εργαζομένων περιλαμβάνει και όσους έχουν
  πρόσφατο ψηφ. ωράριο χωρίς ακόμη `karta_employment` (π.χ. late_check_in
  reply «Χτύπα κάρτα» δεν αποτυγχάνει με ψευδές μη-μοναδικούς). Η εκτέλεση
  κάνει το ίδιο fallback.

---

## 2026-09-16 — Excel ωραρίου: προειδοποιήσεις σύμβασης πριν την υποβολή

- Η προεπισκόπηση ανεβάσματος Excel στο `/ui/schedule` ελέγχει το προτεινόμενο
  εβδομαδιαίο ωράριο με τους ίδιους κανόνες της Αρχικής: επιπλέον εργάσιμες
  ημέρες, υπέρβαση εβδομαδιαίων ωρών και υπέρβαση κανονικής άδειας.
- Οι παραβάσεις εμφανίζονται ομαδοποιημένες σε ένα μήνυμα ανά εργαζόμενο, ακριβώς
  πάνω από το κουμπί «Επιβεβαίωση & αποστολή Ergani», με το ίδιο πλαίσιο
  παράβασης της Αρχικής και πιο συμπαγή γραμματοσειρά.
- Οι προειδοποιήσεις είναι μόνο ενημερωτικές: δεν μπλοκάρουν και δεν αλλάζουν την
  υφιστάμενη διαδικασία υποβολής στο Ergani.
- Διορθώθηκε ο επανυπολογισμός της προεπισκόπησης που έχανε το `import_action`,
  εμφάνιζε λανθασμένα `apply: 0` και άφηνε ανενεργό το κουμπί επιβεβαίωσης.
- Κατά την αποστολή, το κουμπί δείχνει πλέον «Αποστολή σε εξέλιξη…», έχει
  περιστρεφόμενο εικονίδιο και παραμένει ανενεργό για αποφυγή διπλής υποβολής.
- Αν το Ergani απορρίψει το WTOWeek, εμφανίζεται άμεσα το σφάλμα χωρίς τον
  περιττό μεταγενέστερο συγχρονισμό ωραρίου.
- Στις «Καταγραφές» προστέθηκε ξεχωριστό tab «Εισαγωγές Excel»: κάθε μαζικό
  WTOWeek εμφανίζεται μία φορά, μαζί με αρχείο, εβδομάδα, κατάσταση και το κοινό
  πρωτόκολλο της εισαγωγής.
- Επειδή η μαζική υποβολή WTOWeek διακόπτεται από το Ergani στο πρώτο σφάλμα,
  η οθόνη εμφανίζει πλέον ένα μόνο μήνυμα διακοπής και ζητά διορθωμένο Excel,
  αντί να επαναλαμβάνει το ίδιο σφάλμα σε κάθε γραμμή.
- Αφαιρέθηκε η στήλη «Σημειώσεις» από την προεπισκόπηση εισαγωγής. Το μοναδικό
  σφάλμα εμφανίζεται σε κόκκινο πλαίσιο πάνω από τον πίνακα και τα literal `\\n`
  του Ergani αποδίδονται ως κανονικές αλλαγές γραμμής.

---

## 2026-09-16 — Ωράριο: το date picker εβδομάδας δεν κόβεται στο modal

- Στο `/ui/schedule`, το ημερολόγιο της «Επιλογής εβδομάδας Excel» μπορεί πλέον
  να εξέχει από το modal χωρίς να κόβονται οι τελευταίες εβδομάδες του μήνα.
- Ο ειδικός κανόνας του modal έχει μεγαλύτερη CSS ειδικότητα από το γενικό
  `overflow: hidden` των modal panels και ενημερώθηκε το cache version του CSS.
...
- `/ui/work-log/history`: σελιδοποίηση **30** εγγραφές/σελίδα.
- `/ui/schedule` modal επιλογής εβδομάδας Excel: το ημερολόγιο δεν κόβεται
  πλέον (`overflow: visible` στο panel).
- Διόρθωση SyntaxError στο `app/apologistic.py` (χαλασμένο `if/elif` σπαστού)
  που εμπόδιζε εντελώς το φόρτωμα της εφαρμογής.

## 2026-09-16 — WTOWeek: άμεση τοπική ενημέρωση ωραρίου

- Μετά από επιτυχή υποβολή WTOWeek γράφεται τοπικά το `karta_schedule`
  (8 εβδομάδες αν δεν υπάρχει ημερομηνία λήξης), ώστε η Αρχική να δείχνει
  το νέο πρόγραμμα χωρίς αναμονή portal sync.
- Σημείωση: η ισχύς ξεκινά από την «Ισχύς από» — αν είναι μελλοντική
  (π.χ. επόμενη Δευτέρα), η σημερινή Αρχική σωστά δεν τον εμφανίζει ακόμη.

## 2026-09-16 — WebMA μόνο super_admin + login μόνο από βάση

- Η ενότητα «Αλλαγή σύμβασης (WebMA)» στο detail εργαζομένου εμφανίζεται
  και τα API draft/submit επιτρέπονται **μόνο σε super_admin**.
- Κατάργηση login από `.env` (`KARTA_OFFICE_LOGIN_*` / debug admin-ergani)·
  σύνδεση UI μόνο με χρήστες `karta_user` στη βάση.

## 2026-09-16 — WebMA: πλήρης προσυμπλήρωση από EX_BASE_05 / πρόσληψη

- Η μεταβολή σύμβασης προσυμπληρώνει και **ξαναγεμίζει στο submit** όλα τα
  υποχρεωτικά πεδία από τρέχουσα κατάσταση Ergani (`EX_BASE_05`) + τοπική σύμβαση.
- Διόρθωση κωδικού ταυτότητας **ΔΑΤ** (ελληνικά Α/Τ, όχι Latin).
- Χωρίς PDF ουσιωδών όρων δεν στέλνεται πλέον ψευδο-αρχείο `AA==`.
- Επιπλέον mapping: τόπος εργασίας, ΣΣΕ, υποχρ. κατάρτιση, μη προβλέψιμο πρόγραμμα.
...
- Κάθε βράδυ στις **21:00** (scheduled sync) υπολογίζονται οι παραβάσεις
  σύμβασης **για αύριο** (6η μέρα / εβδομαδιαίες ώρες / άδεια) και στέλνονται
  στους λήπτες ειδοποιήσεων του καταστήματος (Telegram + email).
- Env: `KARTA_SCHEDULED_CONTRACT_OVERAGE_NOTIFY_ENABLED`,
  `KARTA_SCHEDULED_CONTRACT_OVERAGE_NOTIFY_TIME` (προεπιλογή `21:00`).
- Μία φορά ανά κατάστημα/ημέρα μέσω sync log
  (`scheduled_contract_overage_notify`).
- Αρχεία: `app/contract_overage_notifications.py`, `app/scheduled_sync.py`,
  `config.py`, `.env.example`.

## 2026-09-15 — Αρχική: alerts παραβάσεων σύμβασης (σήμερα/αύριο)

- Στην `/ui/` (card-report) εμφανίζονται alerts όταν το δηλωμένο ωράριο
  της εβδομάδας δημιουργεί παράβαση:
  - **6η εργάσιμη** σε εργαζόμενο με συμβατικό **5ήμερο**
  - **υπέρβαση εβδομαδιαίων ωρών** — όριο από **Συνολικές ώρες εβδομαδιαίως**
    (`total_weekly_hours`), όχι το μερικό `weekly_hours` (π.χ. Σκόρδα 28ώ όχι 12ώ)
  - **άδεια** ενώ έχουν εξαντληθεί οι δικαιούμενες ημέρες (π.χ. 23η με 22)
- Το μήνυμα ωρών γράφει «N ώρες» (όχι ρολόι τύπου `13:00`).
- Chips στο summary + πορτοκαλί κουτί στην στήλη «Τι να γίνει»· κλικ στο όνομα
  ανοίγει τη σελίδα εργαζομένου.
- Αρχεία: `app/contract_home_alerts.py`, `app/routes_dashboard.py`, `home.js`/`home.html`.

## 2026-09-15 — Κατάλογος ωραρίων απολογιστικού: στήλες χτυπήματος

- Το `scripts/export_apologistic_oraria_katalogos.py` προσθέτει **Χτύπημα από / έως**
  (μοναδικά διαστήματα + αναλυτικά) από το `punch_recorded` του απολογιστικού.

## 2026-09-14 — AI: «Κλείσε τον Τζόλα» μετά τα μεσάνυχτα

- Το «ολα» μέσα σε ονόματα (π.χ. **Τζόλα**) δεν θεωρείται πλέον «κλείσε όλα» —
  μαζικό κλείσιμο μόνο με ολόκληρη λέξη (όλες/όλους/όλα/όσους/ανοιχτές).
- Πριν τις **03:00**, «κλείσε τώρα» χωρίς ανοιχτές σήμερα κοιτάει αυτόματα τις
  χθεσινές ανοιχτές (overnight)· δεν χρειάζεται «από χθες».

## 2026-09-14 — AI: αναγνώριση «κλειστούς/κλειστές»

- Το card intent αναγνωρίζει αιτιατική «κλειστούς/κλειστές/κλειστά».
- «Κλειστούς όλους» / «άνοιξε όλους» μετράει ως νέα εντολή (όχι follow-up σε
  ανοιχτό pending με άλλη ημερομηνία).

## 2026-09-14 — Scanner: παραμονή σύνδεσης μετά το κλείσιμο

- Η συνεδρία scanner διαρκεί **90 ημέρες** με sliding renewal (cookie + SQLite).
- Τα στοιχεία εισόδου αποθηκεύονται στη συσκευή· στο άνοιγμα γίνεται αυτόματη
  επανασύνδεση αν λείπει/έληξε το cookie. Καθαρίζονται μόνο με «Αποσύνδεση».

## 2026-09-14 — Ωρομέτρηση: ταχύτερο Συγκεντρωτικό Excel

- Επανάχρηση styles στο daily sheet· λιγότερη διακόσμηση σε μεγάλα φύλλα
  (>80 γραμμές) ώστε το export να ολοκληρώνει σε ~δευτερόλεπτα αντί λεπτά.

## 2026-09-14 — Συγχρονισμός προσωπικού: επιλογή καταστήματος

- AI εντολή `sync_employees` («συγχρονισμός προσωπικού»): με **πολλά** καταστήματα
  ρωτάει πάντα ποιο (`store_choice`), ακόμα κι αν υπάρχει sticky από τελευταία
  ειδοποίηση· με **ένα** μόνο προχωρά αυτόματα· ρητό όνομα στο μήνυμα («στο Ερατο»)
  παρακάμπτει τη λίστα.
- Το προτεινόμενο κείμενο δείχνει και το όνομα καταστήματος· αγνοούνται τυχόν AFM
  από sticky/LLM (εντολή επιπέδου καταστήματος).

---

## 2026-09-13 — Link αριθμού πρωτοκόλλου → PDF modal

- Στην πραγματική / ιστορικό / ελλιπή κάρτας, ο αριθμός πρωτοκόλλου γίνεται
  σύνδεσμος όταν υπάρχει τοπικό PDF· κλικ ανοίγει το ίδιο modal με `/ui/protocols`.
- Χωρίς αρχείο PDF → απλό κείμενο (όχι link).
- API `GET /api/protocols/by-code/pdf?protocol=…` · enrich `protocol_*_has_pdf` στο
  work-log list/history/missing-cards.

---

## 2026-09-13 — PDF match: overnight έξοδοι (work_date χθες)

- Bug: τα PDF overnight εξόδων αποθηκεύονται στον φάκελο ημερομηνίας υποβολής
  (π.χ. 13/09), ενώ η πραγματική μένει στη `work_date` της βάρδιας (12/09) →
  `no_wl` παρότι ΑΦΜ+ώρα μοναδικά στο PDF (ΡΙΣΤΑ 00:03*, ΦΩΤΟΠΟΥΛΟΣ 01:09* / APERIO).
- Fix: η αντιστοίχιση φορτώνει και την προηγούμενη ημέρα και φιλτράρει με την
  ημέρα γραμμής του PDF.
- Script αναδρομής από τοπικά PDF: `scripts/reapply_protocol_pdf_match_local.py`
  (π.χ. από 2026-08-01 · χωρίς portal download).

---

## 2026-09-12 — Overnight 00:00–02:59: ξανά συγχρονισμός χθεσινής πραγματικής

- Bug: το `run_scheduled_sync` περνούσε πάντα `work_date_iso=σήμερα`, οπότε το
  overnight window (`hour < 3`) δεν ενεργοποιούνταν ποτέ.
- Αποτέλεσμα: έξοδοι με `*` (μετά τα μεσάνυχτα) έμεναν ανοιχτές στην Αρχική μέχρι
  το νυχτερινό 30ήμερο ~03:00 (περίπτωση ΛΑΣΠΑ ΑΡΙΑΔΝΗ / APERIO 11→12/09).
- Fix: overnight ενεργό όταν η ημερομηνία είναι η σημερινή (Αθήνα), ακόμα κι αν
  δόθηκε ρητά ως `work_date_iso`.

---

## 2026-09-12 — Νέα πρόσληψη → QR εντός λεπτών (ανεξάρτητα ωραρίου)

- Το 15λεπτο enrichment συγκρίνει το τρέχον Μητρώο με snapshot ΑΦΜ ανά κατάστημα.
- Νέα εμφάνιση στο Μητρώο (πρόσληψη) κατεβαίνει άμεσα (σύμβαση + QR + σύνδεση),
  ακόμα κι αν δεν υπάρχει ακόμη γραμμή στο ψηφιακό ωράριο.
- Πρώτο snapshot = seed χωρίς μαζικό κατέβασμα· τοπικά κενά QR παραμένουν στόχος.
...
- Οι κανόνες Απολογιστικού και Ωρομέτρησης που εισήχθησαν στις 7/9 και η
  μεταγενέστερη αλλαγή ευελιξίας μερικής απασχόλησης ανακλήθηκαν.
- Η ενεργή συμπεριφορά επέστρεψε στην κατάσταση πριν από το commit `660d04a`.
- Οι νεότερες, άσχετες λειτουργίες Scanner, Excel/WTOWeek, AI Agent,
  πρωτοκόλλων και δικαιωμάτων διατηρήθηκαν.
- Νέα αναγνωριστική έκδοση snapshot: `2026-09-11.rollback-pre-2026-09-07`.
- Οι παλιές καταχωρίσεις του changelog παραμένουν ως ιστορικό των αλλαγών που
  εφαρμόστηκαν και κατόπιν ανακλήθηκαν από την παρούσα καταχώριση.

---

## 2026-09-11 — Αλλαγές ωραρίου: ΑΦΜ + ώρα κάτω από ημερομηνία

- Στήλη **ΑΦΜ**: αριθμός πάνω, όνομα από κάτω (ellipsis)· πλήρες όνομα και στο tooltip.
- Στήλη ώρας: ημερομηνία πάνω, ώρα από κάτω (στενότερη στήλη).
- Cache-bust `?v=20260911sched2`.

## 2026-09-11 — Καταγραφές αλλαγών ωραρίου: σωστή Πηγή (όχι πάντα Excel)

- Bug στο tab αλλαγών ωραρίου: λόγω προτεραιότητας τελεστών JS κάθε γραμμή με
  `source` εμφανιζόταν ως **Excel**. Τώρα φαίνεται σωστά AI Agent / Excel /
  ειδοποίηση κ.λπ.

## 2026-09-11 — Χωρίς AI Agent: ειδοποιήσεις χωρίς κλικ ενέργειας

- Όταν `ai_agent_enabled=0`, οι σημερινές ειδοποιήσεις (Telegram + Email) **δεν**
  περιλαμβάνουν σύνδεσμο/κουμπί «Άνοιγμα ενέργειας» — μόνο ενημέρωση.
- Με AI Agent ενεργό: Telegram παραμένει «Απαντήστε στο μήνυμα για ενέργεια.».

## 2026-09-11 — Ψευδής «καθυστέρηση εισόδου» όταν το portal δεν δίνει Excel

- Όταν το Excel πραγματικής «δεν επέστρεψε αρχείο» και δεν υπάρχει HTML
  fallback, το sync **δεν** αντικαθιστά με κενό και σημαδεύει
  `empty_uncertain`.
- Σε post-sync ειδοποιήσεις, το αυτόματο `late_check_in` **παραλείπεται**
  (`work_log_empty_uncertain`) ώστε να μην στέλνεται ψευδής καθυστέρηση
  (π.χ. Παρδάλη / MALLI CRAFT· είσοδος 09:53, ειδοποίηση 10:30).
...
- Νέο endpoint `GET /listener/setup.rar` που σερβίρει το `listener/setup.rar`
  χωρίς login (attachment download).
- Σύνδεσμος κατεβάσματος στις ρυθμίσεις listener του καταστήματος.

## 2026-09-11 — CardScanner: διόρθωση διπλού χτυπήματος + tab Scanner

- Αν υπάρχει ήδη ίδιο χτύπημα (ίδιος εργαζόμενος / ίδια μέρα / ίδιος τύπος), το
  scanner **δεν το αγνοεί**: εμφανίζει διάλογο επιβεβαίωσης και, αν εγκριθεί,
  ξαναστέλνει ως διόρθωση (`correction_mode`).
- Τα χτυπήματα από scanner σημειώνονται με κανάλι/πηγή **Scanner** στο audit
  και στις καταγραφές υπάρχει νέο tab **Scanner** (`/ui/sync-log#scanner`).

## 2026-09-11 — CardScanner Αποστολές: και δηλώσεις κάρτας

- Οι «Αποστολές» δείχνουν πλέον `karta_work_log` **και** επιτυχημένα
  `karta_card_event`, ώστε ανοιχτό χτύπημα από scanner να φαίνεται πριν τον
  portal sync της πραγματικής.

## 2026-09-11 — AI Agent: σπαστό ωράριο με `/` ή `και`

- Μήνυμα τύπου `11:00-16:00/20:00-23:00` ή `… και …` → `schedule_change` με
  `intervals` (WTODaily πολλαπλά μέρη), όχι ενιαίο ωράριο.
- Πρόταση: «Αλλαγή ωραρίου σε σπαστό 11:00–16:00 · 20:00–23:00».

## 2026-09-11 — AI Agent: «αύριο» δεν ταιριάζει σε επώνυμα

- Stop-words: `αυριο`, `μεθαυριο`, `χθες`, `tomorrow`, `yesterday` (το `σημερα` υπήρχε ήδη·
  το `σήμερα` γίνεται `σημερα` μέσω fold τόνων).
- Πιο αυστηρό fuzzy επωνύμων (μόνο 1 edit)· διορθώνει ψευδοταίριασμα τύπου
  «Αύριο ο Urai» → Κυριμοπούλου + Uraj.

## 2026-09-11 — CardScanner: sync 30″, χρώματα, Αποστολές από βάση

- Αυτόματη αποστολή εκκρεμών κάθε **30″** (όσο η σελίδα είναι ορατή).
- Προσέλευση ανοιχτό πράσινο, Αποχώρηση ανοιχτό κόκκινο.
- «Συγχρονισμός» ενημερώνει «Τελ. συγχρ.» στο μενού.
- «Αποστολές»: ορατό spinner, φρέσκο `no-store` από DB, πρώτη σελίδα **TOP 20**.
- Προαιρετικό `SCANNER_DRY_RUN` στο `.env` / `Config`.
- Service worker **v10**.

## 2026-09-11 — Excel εβδομαδιαίου: ένα WTOWeek για όλους

- Μαζική υποβολή batch: **ένα πρωτόκολλο** για όλους τους εργαζόμενους του Excel
  (`build_wto_week_batch_payload`).

## 2026-09-11 — Κατάλογος κανόνων Απολογιστικού

- Νέο `docs/APOLOGISTIKO_RULES_CATALOG.md`: αναλυτικός κατάλογος `rule_id`
  (είσοδος → συνθήκες → έξοδος → παραδείγματα).

---

## 2026-09-10 — Excel εβδομαδιαίου: WTOWeek ανά εργαζόμενο

- Η επιβεβαίωση εισαγωγής υποβάλλει **WTOWeek** (1 πρωτόκολλο / εργαζόμενο για όλη την εβδομάδα), όχι WTODaily ανά ημέρα.
- Συμπληρωμένες ημέρες με ίδιο ωράριο (`same`) υποβάλλονται ως επιβεβαίωση.
- Στο payload: ΡΕΠΟ→`ΑΝ`, ώρες→`ΕΡΓ`, χωρίς ωράριο/ΡΕΠΟ→`ΜΕ` (Μη εργασία).
- Διόρθωση ανάγνωσης Store ID / AFM / Branch από το φύλλο «Οδηγίες» (αναζήτηση κατά ετικέτα).

---

## 2026-09-10 — CardScanner: επιλογή παραρτήματος στη συσκευή

- Με ≥2 παραρτήματα στον ίδιο χρήστη, η σύνδεση ζητά επιλογή παραρτήματος.
- Η επιλογή αποθηκεύεται στο localStorage της συσκευής και επαναχρησιμοποιείται.
- Αλλαγή από το μπλοκ παραρτήματος στο πλαϊνό μενού.

---

## 2026-09-09 — AI Agent: «άνοιξε όλους» όπως το «κλείσε όλους»

- «άνοιξε όλους/όσους» γεμίζει αυτόματα ΑΦΜ από όσους έχουν **κανονικό ωράριο
  σήμερα** και δεν έχουν ανοίξει κάρτα (όχι ρεπό / χωρίς ωράριο).

---

## 2026-09-09 — AI Agent: ουρά μαζικών «τώρα» χτυπημάτων

- Μαζικό άνοιγμα/κλείσιμο **τώρα** δεν υποβάλλει πλέον όλα στην ίδια wall-clock ώρα.
- Ουρά εκτέλεσης: κάθε επόμενο χτύπημα περιμένει +1–2 λεπτά και στέλνει
  αντίστοιχο `event_at` (ίδια λογική αληθοφάνειας με τα ρητά-ώρα batches).

---

## 2026-09-09 — Accountant: χωρίς ψευδές 403 στις Ρυθμίσεις

- Στις Ρυθμίσεις ο λογιστής δεν καλεί πλέον APIs ειδοποιήσεων/listener
  (που δεν βλέπει)· σταματά το αρχικό «Δεν έχετε δικαίωμα πρόσβασης».

---

## 2026-09-09 — Work-log: διπλή ένδειξη διόρθωσης + overnight merge

- Σε διορθωτικό χτύπημα εμφανίζονται **και οι δύο** ώρες: προηγούμενη διαγραμμένη
  με πρωτόκολλο, νέα κανονικά με πρωτόκολλο (χωρίς badge «διορθ.»).
- Overnight: πραγματική `00:33` δεν χάνει πλέον από κάρτα `23:59` ίδιας βάρδιας·
  κρατάται η μεταγενέστερη πραγματική και η κάρτα φαίνεται διαγραμμένη από κάτω.

---

## 2026-09-09 — Κάρτα: όχι προγενέστερο ίδιο χτύπημα

- Αν υπάρχει ήδη είσοδος ή έξοδος (κάρτα ή πραγματική), απορρίπτεται νέα/διορθωτική
  δήλωση **ίδιου τύπου** με προγενέστερη ώρα (`same_type_earlier`).
- Ισχύει overnight (π.χ. υπάρχουσα έξοδος 00:33 → όχι κάρτα 23:59 στην ίδια βάρδια).
...
- Νέο `/scanner/` με ανεξάρτητη συνεδρία, επαλήθευση web/API ή portal/ΕΦΚΑ χρήστη
  και υποβολή QR μέσω της υπάρχουσας ροής WRKCardSE.
- Πρόσφατα χτυπήματα καταστήματος από τη βάση, 20 ανά σελίδα με προηγούμενο/επόμενο.
- IndexedDB εκκρεμότητες με μετρητή, SQLite deduplication, εκπρόθεσμη αιτιολογία,
  ελεγχόμενες επαναπροσπάθειες και χειροκίνητος συγχρονισμός με διάλογο αποτελέσματος.
- Έλεγχος πρόσβασης στον server ανά 30″, timeout 5″· PIN μόνο για «Αποστολές».
- Πλαϊνό μενού με επίσημη επωνυμία, username portal και τελευταία καταχωρημένη κάρτα.
- Επίσημα λογότυπα, μπλε `#1062fe`, Clock-in/Clock-out, manifest και service worker.
- Τεκμηρίωση `docs/SCANNER_PWA.md`, backend και browser tests. Δεν έγινε πραγματική
  υποβολή κάρτας κατά τις δοκιμές.

## 2026-09-09 — Audit PDF πρωτοκόλλων vs ΒΔ + orphan numeric

- Έλεγχος `protocol_pdfs/` έναντι `karta_ergani_protocol`: 0 orphan ΚΕ/ΟΡ,
  ~8k ελλείποντα (κυρίως παλιά WRKCardSE χωρίς PrintPDF).
- Διαγραφή 102 orphan PDF με αριθμητικό όνομα (`portalId_portalId.pdf`, χωρίς γραμμή ΒΔ).

---

## 2026-09-08 — Διαγραφή καταστήματος: πλήρης purge ΑΦΜ+παραρτήματος

- Νέο `app/store_purge.py`: στη διαγραφή store σβήνονται και δεδομένα με κλειδί
  ΑΦΜ+παράρτημα (`work_log`, ωράριο, κάρτες, συμβάσεις, πρωτόκολλα, απασχολήσεις,
  παράρτημα) και φάκελοι `data/protocol_pdfs/{afm}/{branch}/` +
  `portal_excel_debug/store_{id}/`.
- ΛΑΔΟΚΟΛΛΑ (`803229207`): κρατήθηκε μόνο παράρτημα **4**· καθαρίστηκαν 0 και 6.

---

## 2026-09-08 — Πρωτόκολλα: μόνο παράρτημα καταστήματος

- `/ui/protocols` και sync Excel/ΟΧΕ κρατούν μόνο γραμμές με `branch_aa` ίσο με το
  παράρτημα του ενεργού καταστήματος.
- Καθαρισμός ΒΔ (~23k γραμμές άλλου παραρτήματος) και orphan PDF φακέλων/αρχείων.
...
## 2026-09-09 — CardScanner: εικονίδια και πλοήγηση

- Εικονίδιο εγκατάστασης με το σύμβολο erganiOS σε λευκό φόντο, ξεχωριστές
  εκδόσεις Apple touch / Android maskable και ενημέρωση manifest/cache.
- Επιστροφή από άλλη εφαρμογή στην Αρχική με σωστό τίτλο και επιλεγμένο μενού.
- Αφαίρεση συγχρονισμού από το header· Online/Offline δίπλα στο μενού.
  Ο χειροκίνητος συγχρονισμός παραμένει μέσα στο πλαϊνό μενού.
- Ενημέρωση browser tests για συγχρονισμό από το μενού και επιστροφή στην Αρχική.
...
- Η ένδειξη «Οφείλεται ρεπό» εμφανίζεται μόνο όταν το κατάστημα έχει ενεργή τη
  ρύθμιση μεταφοράς ρεπό λόγω εργασίας την Κυριακή.
- Με ανενεργή ρύθμιση αποκρύπτονται και παλιές εκκρεμότητες που μπορεί να είχαν
  αποθηκευτεί πριν αλλάξει η ρύθμιση του καταστήματος.

---

## 2026-09-07 — Υπερεργασία/υπερωρία αποκλειστικά από τη σύμβαση

- Η ενεργή ημερήσια βάση υπερεργασίας και υπερωρίας προκύπτει πλέον μόνο από
  το συμβατικό 5ήμερο/6ήμερο, χωρίς αλλαγή από τη διάρκεια του δηλωμένου ωραρίου.
- Η ωρομέτρηση υπολογίζει επίσης τη βάση απευθείας από τη σύμβαση και αγνοεί
  τυχόν παλιό, αποθηκευμένο declaration-driven πεδίο.
- Ο προηγούμενος τρόπος διατηρείται ως χωριστή, ανενεργή legacy στρατηγική για
  ελεγχόμενη μελλοντική επανενεργοποίηση.

---

## 2026-09-07 — Ωρομέτρηση: 6η ημέρα πενθημέρου ανεξάρτητα από το 40ωρο

- Σε πλήρη απασχόληση πενθημέρου, έξι αναγνωρισμένες ημέρες εργασίας παράγουν
  πλέον 6η ημέρα ακόμη και όταν η εβδομαδιαία καθαρή βάση δεν ξεπερνά τις
  `40:00`.
- Η 40ή ώρα παραμένει όριο κατάταξης πρόσθετου χρόνου, αλλά δεν αποτελεί πλέον
  προϋπόθεση αναγνώρισης της 6ης διαφορετικής ημέρας.
- Ο ειδικός χειρισμός Κυριακής/7ης ημέρας, τα premium breakdowns και οι
  κατηγορίες επισιτισμού άνω των 48 ωρών διατηρούνται.

---

## 2026-09-07 — Απολογιστικό: μονό χτύπημα σε δηλωμένο σπαστό

- Μονό χτύπημα μέσα σε οποιοδήποτε τμήμα έγκυρου δηλωμένου σπαστού διατηρεί
  ολόκληρο το δηλωμένο ωράριο και χαρακτηρίζεται **Σ**, χωρίς μετάβαση σε έλεγχο.
- Μονό χτύπημα εκτός των τμημάτων αντιστοιχίζεται στο πλησιέστερο τμήμα και
  παράγει **Μ** με τη συμβατική διάρκεια και κενό τουλάχιστον τριών ωρών.
- Η υπερεργασία/υπερωρία του σπαστού υπολογίζεται από το άθροισμα των τμημάτων,
  όχι από το συνολικό χρονικό άνοιγμα πρώτης έναρξης–τελευταίας λήξης.

---

## 2026-09-07 — Fallback Κυριακή→Δευτέρα για επιπλέον ημέρες

- Η εργασία σε ρητό προγραμματισμένο ρεπό παραμένει η πρώτη επιλογή για 6η
  ημέρα πλήρους 5ημέρου και επιπλέον ημέρα εκ περιτροπής.
- Αν δεν υπάρχει ή δεν επαρκεί τέτοια ημέρα, οι υπολειπόμενοι χαρακτηρισμοί
  επιλέγονται από Κυριακή προς Δευτέρα και σημειώνονται με προειδοποίηση.
- Νέα έκδοση: `timekeeping-v18-rest-first-sunday-fallback`.

---

## 2026-09-07 — 6η και επιπλέον ημέρα από εργασία σε προγραμματισμένο ρεπό

- Η 6η ημέρα πλήρους 5ημέρου δεν επιλέγεται πλέον από τη μικρότερη ή πρώτη
  διάρκεια. Ταυτοποιείται από εργασία σε ημέρα που το αρχικό πρόγραμμα είχε
  ΡΕΠΟ/ΑΝΑΠΑΥΣΗ/ΜΗ ΕΡΓΑΣΙΑ.
- Το ίδιο ισχύει για τις επιπλέον ημέρες εκ περιτροπής. Πολλές σαφείς ημέρες
  ρεπό εξετάζονται Δευτέρα→Κυριακή μέχρι να καλυφθεί το εβδομαδιαίο πλεόνασμα.
- Όταν το πρόγραμμα δεν προσδιορίζει μοναδικά/επαρκώς τις ημέρες, εμφανίζεται
  προειδοποίηση και δεν γίνεται αυθαίρετη επιλογή.
- Νέα έκδοση: `timekeeping-v17-scheduled-rest-extra-days`.

---

## 2026-09-07 — 6η ημέρα: αποκλειστικά καθαρή βάση

- Ο χαρακτηρισμός 6ης ημέρας μεταφέρει μόνο την καθαρή βάση στις κατηγορίες
  6ης ημέρας. Υπερεργασία και υπερωρία της ίδιας ημέρας παραμένουν στα δικά
  τους κουτιά και δεν μετατρέπονται σε 120%.
- Ο ίδιος κανόνας εφαρμόζεται σε δηλωμένα 9ωρα/10ωρα και στον επισιτισμό άνω
  των 48 εβδομαδιαίων ωρών. Η 7η Κυριακή παραμένει χωριστή κατ’ εξαίρεση περίπτωση.
- Νέα έκδοση: `timekeeping-v16-sixth-base-exclusive`.

---

## 2026-09-07 — Τεμαχισμός ενιαίου δηλωμένου μεγάλου ωραρίου

- Πλήρες ενιαίο δηλωμένο ή εγκεκριμένο προτεινόμενο ωράριο άνω της ημερήσιας
  βάσης τεμαχίζεται σε βάση, επέκταση ανισομερούς όπου εφαρμόζεται,
  υπερεργασία και υπερωρία, ακόμη και χωρίς χτυπήματα.
- Ένα μικρότερο χτύπημα δεν καταργεί τις πρόσθετες ζώνες που περιλαμβάνει ήδη
  το δηλωμένο/προτεινόμενο μεγάλο ωράριο. Μεγαλύτερη πλήρης κάρτα μπορεί να τις
  επεκτείνει. Ήδη χαρακτηρισμένα λεπτά του Απολογιστικού δεν διπλομετρώνται.
- Καλύφθηκαν με πίνακα δοκιμών 5ήμερο/6ήμερο, ημερήσια όρια, πλήρης ή ελλιπής
  κάρτα, μικρότερο και μεγαλύτερο πραγματικό ωράριο, νύχτα/διανυκτέρευση,
  σπαστό ωράριο, εξωτερικό διάλειμμα και εγκεκριμένη Μεταβολή.
- Προστέθηκε ειδικός πίνακας για δηλωμένο ενιαίο 10ωρο με πλήρη χτυπήματα
  διάρκειας 7:00, 8:00, 8:30, 9:00, 9:30, 10:00 και 11:00: έως το 10ωρο
  διατηρούνται οι δηλωμένες ζώνες, ενώ μεγαλύτερη κάρτα τις επεκτείνει.
- Νέα έκδοση υπολογισμού: `timekeeping-v15-declared-extra-bands`.

---

## 2026-09-07 — Ανισομερής κατανομή χωρίς απορρόφηση υπερεργασίας στη βάση

- Η κοινή βάση κόβεται σε 8:00 στο πλήρες 5ήμερο και 6:40 στο πλήρες 6ήμερο·
  δεν περιέχει υπερεργασία.
- Προστέθηκε χωριστή τετραπλή κατηγορία «Επέκταση ανισομερούς» σε UI και Excel.
- Στην πλήρη ανισομερή απασχόληση, το ημερήσιο πρόσθετο τμήμα κατανέμεται
  χρονολογικά Δευτέρα→Κυριακή: πρώτα ως επέκταση μέχρι να συμπληρωθούν 40:00
  βάσης και επέκτασης την εβδομάδα και κατόπιν ως υπερεργασία.
- Η μερική και η εκ περιτροπής παραμένουν στους ημερήσιους κόφτες 8:00/6:40
  χωρίς επέκταση ανισομερούς· ο χρόνος πάνω από τον κόφτη οδηγείται στο 120%.
- Η ίδια εβδομαδιαία φορά Δευτέρα→Κυριακή εφαρμόζεται πλέον στην κατανομή
  πρόσθετης εργασίας μερικής και στην επιλογή επιπλέον ημερών εκ περιτροπής.
- Νέα έκδοση υπολογισμού: `timekeeping-v14-uneven-extension`.

---

## 2026-09-07 — Στήλες κατ’ εξαίρεση 6ης ημέρας σε Κυριακή/αργία

- Προστέθηκαν στο UI, στο συγκεντρωτικό Excel και στην πλήρη ανάλυση οι στήλες
  «Κατ’ εξαίρεση 6η ημέρα Κυρ/Αργία» και «Κατ’ εξαίρεση 6η ημέρα Κυρ/Αργία νύχτα».
- Η πρώτη παίρνει μόνο τα μη νυχτερινά λεπτά της κατ’ εξαίρεση 6ης ημέρας που
  πέφτουν σε Κυριακή ή ορισμένη αργία. Η δεύτερη παίρνει τα αντίστοιχα λεπτά
  που πέφτουν ταυτόχρονα στη νυχτερινή ζώνη 22:00–06:00.
- Οι νέες στήλες είναι παράγωγες προβολές του υφιστάμενου breakdown και δεν
  προστίθενται ξανά στα σύνολα, ώστε να αποφεύγεται η διπλομέτρηση.
- Νέα έκδοση υπολογισμού: `timekeeping-v13-exception-sixth-holiday-columns`.

---

## 2026-09-07 — Εργαζόμενοι: ημερομηνία τελευταίου επιτυχούς ελέγχου

- Το νέο `last_checked_at` ανανεώνεται σε κάθε επιτυχή ανάγνωση σύμβασης, ακόμη και χωρίς αλλαγή στοιχείων.
- Η ένδειξη «Αποθήκευση έκδοσης» διατηρεί το `synced_at`, χωρίς να επηρεάζεται το ιστορικό εκδόσεων. Η ημερομηνία ενημέρωσης ΕΡΓΑΝΗ παραμένει ανεξάρτητη.
- Πριν την εγκατάσταση εκτελείται το `sql/alter_add_contract_last_checked_at.sql` ή το `scripts/ensure_karta_employment_contract_table.py`. Οι παλιές εγγραφές έχουν κενό τελευταίο έλεγχο μέχρι τον επόμενο επιτυχή συγχρονισμό.
- Έλεγχοι: 10 tests σύμβασης/parser και έλεγχος σύνταξης των τριών επηρεαζόμενων JavaScript αρχείων.

---

## 2026-09-06 — Αρχική: σειρά ονομάτων κατά ώρα εισόδου

- Στη `/ui/` οι εργαζόμενοι εμφανίζονται πρώτα κατά χτύπημα εισόδου (νωρίς→αργά),
  μετά όσοι δεν έχουν χτύπημα κατά έναρξη ψηφιακού ωραρίου, και στο τέλος
  αλφαβητικά όσοι δεν έχουν ούτε τα δύο.

---

## 2026-09-04 — Μηνιαίο αρχείο ψηφιακής οργάνωσης: μόνο τρέχων − 2

- Το scheduled archive (1η του μήνα) τραβάει **μόνο** τον μήνα «τρέχων − 2»
  (π.χ. 1η Σεπ → Ιούλιος). Όχι τον προηγούμενο μήνα, που συχνά δεν είναι ακόμα
  σωστά ενημερωμένος στο Ergani.
- Ιανουάριος/Φεβρουάριος: χωρίς μηνιαίο τράβηγμα.

---

## 2026-09-03 — AI: μερικό όνομα καταστήματος + ασαφή → LLM

- «training room» ταιριάζει με «Training Room Ίλιον» (≥2 λέξεις ή όλες πλην μίας).
- Αν οι κανόνες δεν βγάζουν σίγουρο συμπέρασμα (άγνωστο όνομα κ.λπ.) δεν κόβουν με
  «δεν εντόπισα» — στέλνουν τον διάλογο στο Gemini/OpenAI.

---

## 2026-09-03 — AI Telegram: συγκεκριμένα ονόματα, όχι η προηγούμενη PIN εντολή

- «Ποιος έχει καθυστερημένη είσοδο/έξοδο» απαντά από την Αρχική
  (`late_arrival` / `needs_checkout`), όχι ως εντολή κάρτας.
- «Άνοιξε γκουμα» (χωρίς τη λέξη κάρτα) είναι νέα εντολή εισόδου. Αν υπάρχει
  ανοιχτό PIN άλλου καταστήματος/άλλων ατόμων, ακυρώνεται.
- Πολλά επώνυμα στο ίδιο μήνυμα («άνοιξε γκουμα μπασιο») χωρίζονται σε λέξεις,
  δένονται με τα πλησιέστερα ονόματα της προηγούμενης λίστας και παίρνουν την
  ίδια ενέργεια. «Εννοείτε του Χ ή του Υ;» μόνο όταν το **ίδιο** επώνυμο ταιριάζει
  σε πολλούς.

---

## 2026-09-03 — Αρχική: ρεπό με χτύπημα εμφανίζεται «Σε εργασία»

- Αν το ψηφιακό ωράριο είναι ρεπό/ανάπαυση αλλά υπάρχει έναρξη (πραγματική ή κάρτα)
  χωρίς έξοδο, το status είναι `at_work` / **Σε εργασία**, όχι «Ανάπαυση / ρεπό».
- Η ενέργεια παραμένει «δήλωση αποχώρησης». Με είσοδο και έξοδο η ημέρα μένει
  «Ολοκληρωμένη μέρα».

---

## 2026-09-03 — Ελλιπές χτύπημα: χωρίς υπερωρία από τεκμαρτό όριο

- Μόνο έναρξη: μετά την πρόταση (πραγματική έναρξη + δηλωμένη διάρκεια) η
  τεκμαρτή λήξη κόβεται στη νέα δηλωμένη λήξη. Π.χ. δηλωμένο `13:00–21:00`,
  χτύπημα `09:00–` → `09:00–17:00`, χωρίς υπερωρία.
- Μόνο λήξη με επιπλέον ώρες: η πρόταση παραμένει ανάστροφα από την πραγματική
  λήξη (`13:00–21:00`, `–21:05` → `13:05–21:05`).
- Έκδοση υπολογισμού: `2026-09-03.rules-v13-no-ot-1punch` (συντομευμένη ώστε να
  χωράει στη στήλη `calculation_version`).

---

## 2026-09-03 — Μαζική υποβολή απολογιστικού: ένα πρωτόκολλο ανά τύπο

- Η μαζική καταχώρηση στέλνει πλέον **μία δήλωση Ergani ανά τύπο εγγράφου**
  (WTODailyA / WTOLeave / WTOOvA) με πολλούς εργαζομένους, αντί για loop ανά γραμμή.
- Νέο `POST /api/apologistic/submit-bulk` και `app/apologistic_batch_submit.py`.
- Το ίδιο πρωτόκολλο γράφεται σε πολλές γραμμές `karta_apologistic_submit`.

---

## 2026-09-03 — Sync-log Απολογιστικό: διόρθωση σελιδοποίησης

- Η επόμενη σελίδα στο tab `#apologistic` απέτυχε ή επαναλάμβανε την πρώτη:
  λάθος σειρά παραμέτρων στο `TOP (?)` και σύγκριση `datetimeoffset` με naive UTC.
- Το cursor φιλτράρει πλέον με `CAST(... AS datetime2(0))` και σωστό binding.

---

## 2026-09-03 — UI: κάθετοι τίτλοι απολογιστικού και modal QR εργαζομένων

- Οι κάθετοι τίτλοι του πίνακα Απολογιστικού χρησιμοποιούν `writing-mode` στο
  εσωτερικό label (χωρίς `max-width`/`ellipsis` που έκοβε τα γράμματα).
- Στο `/ui/employees`, το modal QR έχει πεδία το ένα κάτω από το άλλο, χωρίς bold,
  σε ύφος `field-label`/`field-input`· το πλαίσιο QR ευθυγραμμίζεται κάτω με το
  τελευταίο πεδίο (χωρίς τέντωμα).

---

## 2026-08-31 — Απολογιστικό: διόρθωση εμφάνισης σε Safari

- Τα κάθετα headers του πίνακα Απολογιστικού δεν βασίζονται πλέον σε `writing-mode`
  πάνω στο ίδιο το `th`, ώστε ο Safari να μην αυξάνει υπερβολικά το ύψος των γραμμών.
- Η περιστροφή γίνεται σε εσωτερικό label του header και οι γραμμές του πίνακα
  κρατούν κανονικό ύψος όπως στο Chrome.

---

## 2026-08-31 — Accountant: μενού Καταστήματα για επιλογή

- Ο ρόλος `accountant` βλέπει πλέον το nav **Καταστήματα** (`stores.view`) ώστε να
  επιλέγει ενεργό κατάστημα από τα επιτρεπόμενά του.
- Χωρίς `stores.manage` / `stores.credentials.manage`: κρύβονται Νέο / Επεξεργασία /
  Διαγραφή· μένουν Επιλογή και (όπου επιτρέπεται) Ρυθμίσεις αργιών.

---

## 2026-08-31 — Accountant: μαζική υποβολή απολογιστικού στο Ergani

- Ο ρόλος `accountant` έχει ρητά `schedule.submit_daily` (μεμονωμένες και μαζικές
  υποβολές WTODailyA / WTOOvA από το Απολογιστικό).
- Τα δικαιώματα ρόλου ισχύουν πάντα ως βάση· τα `karta_user_permission` προσθέτουν
  μόνο managed extras, ώστε να μην «χάνεται» η υποβολή από παλιό session/λίστα χρήστη.

---

## 2026-08-31 — Μαζική Καταχώρηση: υποβολή μετά την προεπισκόπηση

- Το modal **Μαζική Καταχώρηση** επιτρέπει πλέον αποστολή στο Ergani μετά την
  προεπισκόπηση (WTODailyA / WTOOvA εκκρεμών μεταβολών).
- Οι εγγραφές **Ελέγχου** εξακολουθούν να εξαιρούνται (απαιτείται έγκριση πρώτα).
...
- Reply σε Telegram notification κληρονομεί το κατάστημα και από το εμφανές
  κείμενο του notification όταν λείπει το αποθηκευμένο `store_id`.
- Μετά από αριθμητική/ονομαστική επιλογή καταστήματος, ο parser και η απάντηση
  περιορίζονται αποκλειστικά στα δεδομένα του επιλεγμένου καταστήματος.

---

## 2026-08-27 — Απόρριψη εξόδου πριν από την είσοδο

- Νέος έλεγχος `checkout_before_entry_blocked_reason`: η ώρα εξόδου δεν μπορεί να
  είναι πριν ή ίδια με την είσοδο της μέρας εργασίας (κάρτα ή ανοιχτή πραγματική).
- Ισχύει σε Telegram/assistant (`new_card_punch_blocked_reason`) και UI κάρτας.
- Overnight έξοδοι (επόμενη ημερολογιακή μέρα μετά το normalize) παραμένουν έγκυρες.

---

## 2026-08-26 — AI: σχετικές ώρες δεν μετατρέπονται σε «τώρα»

- Οι φράσεις «μία/Χ ώρες πριν», «πριν μία/Χ ώρες» και συνδυασμοί ωρών/λεπτών
  παράγουν retro ώρα από το τρέχον ρολόι.
- Αν υπάρχει «πριν» αλλά η χρονική αναφορά δεν αναλυθεί με βεβαιότητα, το fast
  path δεν επιτρέπεται να τη μετατρέψει σε `_now`· η εντολή συνεχίζει στο LLM.

---

## 2026-08-26 — AI card fast path + token cache + κανόνας αιτιολογίας 15′

- Σαφείς εντολές κάρτας με επιλυμένο εργαζόμενο αναλύονται deterministic πριν
  από Gemini/OpenAI και περνούν από το ίδιο validation/confirmation.
- Process-local cache του access token ΕΡΓΑΝΗ ανά κατάστημα, περιβάλλον και
  credentials, με λήξη πριν από το expiration του server.
- Τα αποτελέσματα εκτέλεσης καταγράφουν `timings_ms` για authentication,
  commands και συνολικό χρόνο.
- Η αιτιολογία WRKCardSE εξαρτάται αποκλειστικά από `submitted_at - event_at`:
  έως και 15′ χωρίς αιτιολογία, πάνω από 15′ με `001`, για κάθε κανάλι και τύπο
  χτυπήματος. Ψηφιακό ωράριο και `flex_arrival_minutes` δεν συμμετέχουν.
- Ο listener επαναλαμβάνει χωρίς αιτιολογία όταν το ΕΡΓΑΝΗ χαρακτηρίσει την
  κίνηση εμπρόθεσμη.
...
- Αν το LLM γυρίσει `commands: []` μαζί με top-level `today_info`/intent,
  δεν εμφανίζεται πια «Δεν αναγνωρίστηκε καμία εντολή» — επικυρώνεται ως μία εντολή.

---

## 2026-08-25 — AI: LLM πρώτα, κανόνες τελευταία επιλογή

- Όλα τα ερωτήματα/εντολές περνούν πρώτα από Gemini→OpenAI· οι τοπικοί
  κανόνες (today_info, κάρτα, «ποιος τελειώνει…») μόνο αν αποτύχουν τα LLM.

---

## 2026-08-25 — AI: «ποιος τελειώνει στις…» χωρίς LLM + μεγαλύτερα timeouts

- Κανόνας για τελειώνει/ξεκινά στις HH:MM (και `19.40`) από `today_home` (fallback).
...
- Αν πέσει το Gemini, το OpenAI δοκιμάζεται **πάντα** (δικό του timeout 6s)·
  δεν παραλείπεται λόγω wall. Wall default 14s για το Gemini slot.

---

## 2026-08-25 — AI: κανόνες για άνοιξε/κλείσε κάρτα (χωρίς LLM)

- `άνοιξε/κλείσε κάρτα`, `clock in/out`, `είσοδος/έξοδος` → rule-first όπως τα
  today_info: ονόματα, όλες/ανοιχτές, ώρα retro, focus, διευκρίνιση ομωνύμων.
- Κοινό wall 8s για LLM παραμένει για ό,τι δεν καλύπτουν οι κανόνες.

---

## 2026-08-25 — AI: όχι στοίβαγμα Gemini+GPT (latency ≤ ~8s)

- Απλά `today_info` («ποιοι εργάζονται», ανοιχτές κάρτες) → **κανόνες πρώτα**, χωρίς LLM.
- Κοινό wall `ASSISTANT_LLM_WALL_SEC` (default 8s): αν το Gemini εξαντλήσει τον χρόνο,
  **δεν** ξεκινά OpenAI για άλλα ~20s.
- OpenAI timeout default 6s· μόνο αν μένει αρκετός χρόνος στο wall.

---

## 2026-08-24 — Απολογιστικό: απόκρυψη άδειας χωρίς χτύπημα

- Ημέρα με δηλωμένη άδεια και κανένα χτύπημα κάρτας εξαιρείται πλήρως από το
  Απολογιστικό, καθώς δεν απαιτεί ενέργεια ή ενημερωτική αναφορά.
- Άδεια με έστω ένα χτύπημα εξακολουθεί να εμφανίζεται για έλεγχο.

---

## 2026-08-23 — Ωρομέτρηση Excel: άδειες και εφαρμοζόμενο ωράριο

- Οι περιγραφές Ergani όπως «Κανονική άδεια» κανονικοποιούνται χωρίς ελληνικούς
  τόνους πριν από τον χαρακτηρισμό. Έτσι δεν περνούν πλέον ως «Χωρίς δηλωμένο
  ωράριο» και εξαιρούνται από την ωρομέτρηση.
- Στην «Πλήρη ανάλυση» η στήλη «Δηλωμένο / προτεινόμενο ωράριο» περιέχει μόνο
  ώρες: προτεινόμενο στη μεταβολή, δηλωμένο στη σταθερή περίπτωση και κενό για
  άδεια, ρεπό, μη εργασία ή άλλη λεκτική κατάσταση. Δεν προστέθηκε στήλη Σ/Μ.
- Νέα έκδοση υπολογισμού `timekeeping-v9-visible-base-cap`.
- Ο ημερήσιος κόφτης περιορίζει πλέον και το εμφανιζόμενο αναγνωρισμένο ωράριο,
  όχι μόνο τα καθαρά μισθολογικά λεπτά. Διατηρείται επιπλέον μόνο η χρονική
  επέκταση εξωτερικού διαλείμματος, χωρίς αύξηση καθαρής εργασίας.
- Προστέθηκαν ρητοί κανόνες τοποθέτησης βάσης για πρόωρη έναρξη, καθυστερημένο
  σύντομο/πλήρες χτύπημα, λήξη εντός δηλωμένου ορίου και διανυκτέρευση.
- Η εξαίρεση «διάρκεια πρότασης = διάρκεια χτυπήματος» περιορίστηκε ρητά στη
  μεταβολή από χωρίς ωράριο/ρεπό/μη εργασία σε εργασία, όταν δεν υπάρχει ημέρα
  ανταλλαγής. Καταγράφεται και ο ακριβής κανόνας διάρκειας ανά είδος σύμβασης.

---

## 2026-08-23 — AI/guards: κλείσιμο μόνο ανοιχτών + απόρριψη ήδη κλειστής πραγματικής

- «κλείσε όλες» παίρνει ΑΦΜ μόνο από πραγματικά ανοιχτές της ημερομηνίας.
- Guard: αν η πραγματική έχει ήδη Από+Έως (π.χ. ΤΟΥΜΑΡΑΣ 13:59–20:00) χωρίς
  WRKCardSE out, το κλείσιμο απορρίπτεται soft.

---

## 2026-08-23 — AI: today_home περιλαμβάνει χθες (overnight ανοιχτές)

- Το snapshot για τον assistant έχει πλήρη εικόνα **σήμερα** και **χθες μόνο
  ανοιχτές κάρτες**, ώστε ερωτήσεις/κλείσιμο «ανοιχτές χθες» μετά τα μεσάνυχτα
  να βασίζονται σε πραγματικά δεδομένα, όχι σε «δεν υπάρχουν διαθέσιμα».

---

## 2026-08-23 — Ωρομέτρηση: κόφτης πλήρους βάσης και ακεραιότητα προσαυξήσεων

- Σε πλήρη απασχόληση η αναγνωρισμένη καθαρή βάση κόβεται πλέον στην ημερήσια
  βάση που επέλεξε το απολογιστικό: ακριβώς 8:00 ή 6:40 από τη δήλωση και, για
  κάθε άλλη δηλωμένη διάρκεια, 8:00 στο συμβατικό 5ήμερο ή 6:40 στο 6ήμερο.
- Δήλωση άνω των 8 ωρών που ταυτίζεται με το χτύπημα δεν ενσωματώνει πλέον την
  υπερεργασία/υπερωρία στην κοινή βάση και δεν προκαλεί διπλομέτρηση.
- Η επέκταση διαλείμματος εκτός ωραρίου παραμένει στη χρονική έκταση της βάσης,
  χωρίς να αυξάνει τα καθαρά λεπτά εργασίας.
- Οι νυχτερινές και Κυριακής/αργίας προσαυξήσεις του πραγματικού βασικού
  τμήματος δεν μπορούν να χαθούν λόγω διατήρησης διαφορετικού δηλωμένου
  χρονικού διαστήματος. Εξαιρούνται πρώτα υπερεργασία, υπερωρία και υπέρβαση.
- Νέα έκδοση υπολογισμού `timekeeping-v7-full-base-cap` και regression tests για
  δηλωμένο 9ωρο/10ωρο, βάση 6:40 και προστασία πραγματικών προσαυξήσεων.

---

## 2026-08-22 — Κανονική άδεια στη λίστα εργαζομένων

- Το `GET /api/employees/list` επιστρέφει για κάθε εργαζόμενο τις ημέρες κανονικής
  άδειας που έχουν ληφθεί από 1/1 του τρέχοντος έτους και τις δικαιούμενες ημέρες,
  βάσει του `karta_portal_schedule_archive`.
- Στο `/ui/employees` προστέθηκε στήλη «Άδεια» με μορφή `ληφθείσες/δικαιούμενες`
  (π.χ. `3/22`) και υπότιτλο τον τελευταίο διαθέσιμο μήνα του αρχείου (π.χ.
  `06/2026`) ή παύλα όταν δεν υπάρχουν διαθέσιμα στοιχεία από το Ergani.
- Οι ληφθείσες ημέρες προκύπτουν από τη νεότερη σωρευτική τιμή του Ergani και
  δεν αθροίζονται ανά ημερήσια γραμμή. Η ετήσια βάση είναι 22 ημέρες στο 5ήμερο
  και 26 ημέρες στο 6ήμερο.

---

## 2026-08-22 — AI: sticky κατάστημα μετά από today_info

- Follow-up χωρίς reply («κλείσε όσους τελειώνουν…») κρατάει το κατάστημα
  του προηγούμενου outbound (π.χ. Ερατο) αντί να ξαναρωτάει λίστα καταστημάτων.

---

## 2026-08-22 — Batch χτυπημάτων: ώρα ±1–2 λεπτά ανά εγγραφή

- Σε πολλαπλές υποβολές κάρτας (AI agent, UI, auto-close) η ώρα χτυπήματος
  (`event_at`) αποστέλλεται με διαφορά 1–2 λεπτά ανά χτύπημα.
- 1ο στη βάση (ρητή ώρα ή «τώρα»), κάθε επόμενο +1–2 λεπτά.
  Από 2026-09-09 το live/τώρα στον AI Agent έχει και πραγματική ουρά αναμονής.

---

## 2026-08-22 — AI: επεξηγηματικές απαντήσεις όταν απορρίπτεται κάρτα

- Όταν δεν γίνεται άνοιγμα/κλείσιμο, το bot αναφέρει τον λόγο με ώρα
  (π.χ. «Δεν γίνεται άνοιγμα — υπάρχει ήδη δήλωση εισόδου στις 12:40.»).

---

## 2026-08-22 — AI: διόρθωση λάθους είσοδου/εξόδου κάρτας

- Ενισχυμένο prompt: «άνοιξε» ποτέ `check_out`, «κλείσε» ποτέ `check_in`.
- Safety net στο validation: αν το LLM δώσει αντίθετο intent από ρητά ρήματα
  στο μήνυμα, διορθώνεται αυτόματα πριν την απάντηση/εκτέλεση.

---

## 2026-08-21 — LLM: «πριν Χ λεπτά» = από τώρα

- Στο prompt του assistant διευκρινίζεται ότι «πριν Χ λεπτά» υπολογίζεται από το
  `now`, όχι από την ώρα εισόδου/εξόδου της κάρτας· offset από χτύπημα μόνο με
  ρητή φράση («Χ λεπτά πριν την είσοδο/έξοδο»).

---

## 2026-08-21 — Ρόλος Accountant (λογιστής)

- Νέος ρόλος `accountant`: βλέπει Αρχική, ωράριο, πραγματική απασχόληση,
  πρωτόκολλα, απολογιστικό, ελλειπή χτυπήματα, ψηφιακή κάρτα, εργαζόμενους
  και Ρυθμίσεις· όχι Συγχρονισμό, Καταστήματα, Καταγραφές, Χρήστες.
- Στις Ρυθμίσεις ο λογιστής βλέπει/επεξεργάζεται μόνο τις Αργίες
  (`settings.holidays.view` / `settings.holidays.edit`).
- Τα API αργιών έχουν ρητά permissions· η λίστα καταστημάτων για picker
  βασίζεται σε `stores.select` (χωρίς nav Καταστήματα).

---

## 2026-08-21 — Διάρκειες ωρομέτρησης σε δεκαδικές ώρες

- UI, συγκεντρωτικό Excel και πλήρης ανάλυση Excel εμφανίζουν πλέον τις
  ποσότητες χρόνου σε δεκαδικές ώρες αντί για μορφή `ωω:λλ`.
- Παράδειγμα: 90 λεπτά εμφανίζονται ως `1,5` ώρες.
- Στα Excel οι ώρες αποθηκεύονται ως αριθμητικές τιμές με μορφή `0.##`, ώστε να
  παραμένουν διαθέσιμες για άθροιση και περαιτέρω υπολογισμούς.
- Τα ίδια τα ωράρια και χρονικά διαστήματα, π.χ. `09:00–17:00`, παραμένουν σε
  μορφή ώρας και δεν μετατρέπονται.

---

## 2026-08-21 — Καθαρή βάση και διάλειμμα ωρομέτρησης

- Η καθαρή βάση αφαιρεί διάλειμμα μόνο όταν υπάρχει ρητή ένδειξη «εκτός χρόνου
  εργασίας» (`break_in_work=0`).
- Διάλειμμα εντός ωραρίου δεν αφαιρείται: καθαρή βάση = αναγνωρισμένη βάση
  υπολογισμού ωραρίου.
- Το εκτός διάλειμμα δεν επεκτείνει πλέον τεχνητά τη λήξη του αναγνωρισμένου
  ωραρίου πριν αφαιρεθεί.
- Επιβεβαιώθηκε με tests ότι σε Μεταβολή η Πρόταση αντικαθιστά το δηλωμένο
  ωράριο πριν από τον υπολογισμό καθαρής βάσης και τους εβδομαδιαίους κανόνες.
- Νέα έκδοση υπολογισμού: `timekeeping-v6-break-basis`.
...
## 2026-08-21 — Απολογιστικό: ρητή επιλογή ανταλλαγής και απόρριψη Ε/Μ σε Σ*

- Το μεμονωμένο κλικ στο **Ε**, όταν υπάρχουν επιλογές ανταλλαγής, ζητά ρητά
  «Μόνο αυτή η ημέρα». Η ανταλλαγή εφαρμόζεται αποκλειστικά με κλικ στη
  συγκεκριμένη κάρτα ανταλλαγής και δημιουργεί τις δύο συνδεδεμένες μεταβολές
  **Μ***· δεν επιλέγεται πλέον αυτόματα.
- Η «Έγκριση όλων προς μεταβολή» διατηρεί την αυτόματη επιλογή μίας διαθέσιμης
  ανταλλαγής με χρονολογική προτεραιότητα.
- Προστέθηκε ενέργεια **Α (Απόρριψη)** σε όλα τα **Ε** και στα απλά **Μ**· η
  εγγραφή αποθηκεύεται ως **Σ***. Η ενέργεια δεν εμφανίζεται στα **Μ***.
- Τα **Σ*** μπορούν να επανέλθουν στην αρχική αποθηκευμένη κατάσταση (Ε ή Μ).
- Η μαζική καταχώρηση ΕΡΓΑΝΗ συνεχίζει να περιλαμβάνει αποκλειστικά εγγραφές
  κατάστασης `change`, επομένως τα **Σ** και **Σ*** εξαιρούνται.
...
- **1 κατάστημα** → προχωρά αυτόματα· **πολλά** → όνομα στο μήνυμα (greeklish)·
  αλλιώς λίστα επιλογής (`store_choice`) με κράτημα του αρχικού μηνύματος.
- Απάντηση με αριθμό ή όνομα → επανάληψη του αρχικού μηνύματος στο επιλεγμένο
  κατάστημα (ίδιο task). Reply με `focus_locked` κρατά κατάστημα· απλό sticky
  χωρίς reply δεν αντικαθιστά τη λίστα.
- Docs: `docs/TELEGRAM_ASSISTANT.md` § Επίλυση καταστήματος.

---

## 2026-08-20 — AI Agent: εφεδρικό τοπικό LLM + rule fallback

- Όταν το Gemini αποτύχει (503/timeout), δοκιμάζεται **τοπικό Ollama**
  (`LOCAL_LLM_*`, default `qwen2.5:3b-instruct` στο `127.0.0.1:11434`).
- **`ASSISTANT_LLM_ORDER`**: εναλλαγή πρωτεύοντος/εφεδρικού από `.env`
  (`gemini,local` ή `local,gemini` ή μόνο ένα).
- Αν αποτύχουν τα LLM: **rule-based** απάντηση για απλές ερωτήσεις σήμερα
  (`ASSISTANT_RULE_FALLBACK_ENABLED`, ανοιχτές κάρτες / ποιοι εργάζονται).
- Gemini: 2 attempts ανά μοντέλο σε 429/5xx πριν το επόμενο στη σειρά.

---

## 2026-08-20 — AI Agent: ανοιχτές εντολές ανά κατάστημα + λήξη 3 λεπτά

- Ρητή αναφορά καταστήματος («στο Ερατο…») δένει μόνο σε ανοιχτή εντολή **εκείνου**
  του καταστήματος· δεν συνεχίζει pending άλλου μαγαζιού. Το ίδιο ισχύει για
  `today_home` / focus: το όνομα στο μήνυμα υπερισχύει του προηγούμενου focus.
- Matching ονόματος καταστήματος: ελληνικά/greeklish («Ερατο», «Ερατοσθένους») → `ERATO`.
- Ανοιχτές εντολές με `updated_at` > 3 λεπτά ακυρώνονται αυτόματα (`expired_idle`).
- «άκυρο» / «ακύρωσε» / `cancel` ακυρώνουν **τοπικά** χωρίς Gemini (ώστε 503/timeout
  να μην εμφανίζει «Δεν μπόρεσα να αναλύσω»).

---

## 2026-08-20 — Νέο κατάστημα: αρχείο ψηφιακής οργάνωσης YTD

- Μετά το period sync των 30 ημερών, τελευταίο βήμα εισαγωγής: κατέβασμα από
  «Τρέχουσα Κατάσταση» στο `karta_portal_schedule_archive` από **1/1** του έτους
  έως και **2 μήνες πριν** (π.χ. Αύγουστος → Ιαν–Ιούν).
- Είσοδος Ιανουαρίου/Φεβρουαρίου → παράλειψη (δεν υπάρχουν ακόμη μήνες στο εύρος).
- Φαίνεται στη ροή προόδου wizard (βήμα 3/3) και στις Καταγραφές ως
  `new_store_schedule_archive`.

---

## 2026-08-21 — Το 120% της μερικής μεταφέρθηκε στις υπερωρίες

- Διατηρείται το χρονικό όριο βάσης μερικής: έως 8:00 στο 5ήμερο και έως 6:40
  στο 6ήμερο.
- Ο χρόνος πάνω από το όριο δεν αποτελεί πλέον ξεχωριστή κατηγορία «Μερική
  120%». Εμφανίζεται αποκλειστικά στην κοινή κατηγορία «Υπερωρία 120%».
- Αφαιρέθηκε η ξεχωριστή στήλη «Μερική 120%» από UI και τα δύο Excel.
- Προστέθηκαν χωριστοί έλεγχοι ακεραιότητας για τη βάση μερικής και για τον
  συνολικό αναγνωρισμένο χρόνο μαζί με την υπερωρία 120%.

---

## 2026-08-20 — Today-alert CTA όταν AI Agent = ON

- Με `ai_agent_enabled` στο κατάστημα, το Telegram μήνυμα καθυστερήσεων/today-alert δεν βάζει σύνδεσμο `today-hit`· λέει «Απαντήστε στο μήνυμα για ενέργεια.»
- Με AI Agent = OFF παραμένει «Προχωρήστε σε ενέργεια:» + URL (όπως πριν).

---

## 2026-08-20 — AI Agent: skip ήδη ανοιχτής/κλειστής κάρτας (και πραγματική)

- Ομαδικό άνοιγμα: παραλείπει όσους έχουν ήδη είσοδο κάρτας **ή** ανοιχτή πραγματική απασχόληση (ίδια εικόνα με την Αρχική).
- Ομαδικό κλείσιμο: παραλείπει όσους έχουν ήδη κλείσει· σαφής λίστα «Παραλείφθηκαν».
- Αν παραλειφθούν όλοι → ενημερωτική απάντηση χωρίς επιβεβαίωση προς ΕΡΓΑΝΗ.

---

## 2026-08-20 — AI Agent: απάντηση ≤10s

- Σκληρό budget ~8s για όλα τα Gemini attempts (χωρίς αργό failover που έφτανε 25–40s).
- Cache 45s για `today_home` ανά κατάστημα/ημέρα.

---

## 2026-08-20 — AI Agent: today_home πάντα στο prompt

- Το `today_home` (εικόνα Αρχικής σήμερα) χτίζεται και στέλνεται σε **κάθε** μήνυμα προς το Gemini· δεν παραλείπεται πλέον για απλές εντολές κάρτας.

---

## 2026-08-20 — Αποκλειστική κατανομή βάσης και επιπλέον ημέρες εκ περιτροπής

- Η ωρομέτρηση αναβαθμίστηκε σε `timekeeping-v4-exclusive-base`.
- Τα λεπτά 6ης ημέρας, πρόσθετης μερικής 12% και μερικής 120% αφαιρούνται από
  τις κοινές προσαυξήσεις βάσης και παραμένουν μόνο στο breakdown της ειδικής
  κατηγορίας τους.
- Προστέθηκε ημερήσιος έλεγχος ακεραιότητας: κοινή βάση + 6η ημέρα + πρόσθετη
  μερικής + μερική 120% = συνολική καθαρή αναγνωρισμένη βάση.
- Στην εκ περιτροπής απασχόληση, οι επιπλέον αναγνωρισμένες ημέρες επιλέγονται
  από Κυριακή προς Δευτέρα και μπορούν να είναι περισσότερες από μία.
- Η μερική δεν παράγει 40%/60%: μετά τις 8:00 ή 6:40 παράγει μόνο μερική 120%.
  Στην πλήρη απασχόληση, υπερωρία πέρα από τις 4 ημερήσιες ώρες παραμένει 120%.

---

## 2026-08-20 — AI Agent: χωρίς Python safety-net ονομάτων

- Αφαίρεση του Python επανελέγχου ασάφειας επωνύμου· τα ΑΦΜ τα αποφασίζει το Gemini βάσει `guide`.
- Πολλά διακριτά ονόματα → όλα τα ΑΦΜ· «Εννοείτε…» μόνο όταν το μοντέλο κρίνει ίδιο/παρόμοιο επώνυμο (`ambiguous_employee_afms` στο schema).

---

## 2026-08-20 — Viewer: Excel πλήρους ανάλυσης ωρομέτρησης

- Τα endpoints `timekeeping/export` και `timekeeping/export-detailed` απαιτούν πλέον `work_log.view` (όπως το preview), ώστε `viewer` / `store_viewer` να κατεβάζουν συγκεντρωτικό και πλήρη ανάλυση Excel στο απολογιστικό.

---

## 2026-08-20 — AI Agent: ακύρωση μηδενίζει τη συζήτηση

- Μετά από ακύρωση (`cancel_pending` ή ναι σε «να ακυρώσω;»), κλείνουν όλες οι ανοιχτές εντολές (διευκρίνιση/draft/PIN) και το conversation focus κρατά μόνο το κατάστημα — χωρίς προηγούμενα πρόσωπα.
- Το outbound γράφεται ως `assistant_cancelled` με `reset_conversation_focus`, ώστε το επόμενο μήνυμα να μην κληρονομεί ονόματα/ΑΦΜ.

---

## 2026-08-20 — Πλήρης ανάλυση προσαυξήσεων ανά χρονική ζώνη

- Η ωρομέτρηση αναβαθμίστηκε σε `timekeeping-v3-premium-breakdowns`.
- Αναγνωρισμένη βάση, υπερεργασία 20%, πρόσθετη εργασία μερικής 12%, μερική 120%, υπερωρίες 40%/60%/120% και 6η ημέρα 30% διαχωρίζονται πλέον σε Ημέρας, Νύχτας, Κυριακής/Αργίας και Νύχτας/Κυριακής-Αργίας.
- Ο επιμερισμός γίνεται πάνω στα πραγματικά χρονικά λεπτά κάθε κατηγορίας και όχι με συνδυασμό ανεξάρτητων συνολικών αθροισμάτων.
- Η άδεια εξαιρείται πλήρως από ημέρες, σύνολα και υπολογισμούς ωρομέτρησης.
- Η προειδοποίηση αδυναμίας τοποθέτησης διαλείμματος δεν παράγεται πλέον σε Ρεπό, Μη Εργασία ή άλλη ημέρα χωρίς αναγνωρισμένο χρονικό διάστημα.
- UI, συγκεντρωτικό Excel και πλήρης ανάλυση Excel προβάλλουν τα ίδια canonical breakdowns.

---

## 2026-08-20 — Δεύτερο αναλυτικό Excel ωρομέτρησης

- Η σελίδα ωρομέτρησης προσφέρει δύο λήψεις: **Συγκεντρωτικό Excel** και **Πλήρης ανάλυση Excel**.
- Το αναλυτικό αρχείο ακολουθεί τη δομή του δείγματος με μία γραμμή ανά εργαζόμενο/ημέρα και ομαδοποιημένες στήλες για επιχείρηση, ημερήσια στοιχεία, προσαυξήσεις, υπερεργασία/μερική, υπερωρίες και 6η ημέρα.
- Και τα δύο αρχεία είναι διαφορετικές προβολές του ίδιου `build_timekeeping_report`, χωρίς δεύτερο υπολογιστικό μηχανισμό.
- Η πρόσθετη εργασία μερικής κατανέμεται πλέον με την οριστική σειρά **Κυριακή → Δευτέρα**. Τα λεπτά τοποθετούνται από τη λήξη του αναγνωρισμένου ωραρίου προς τα πίσω και το ακριβές διάστημα μεταφέρεται στην πλήρη ανάλυση.
- Αν υπάρχει θετική εβδομαδιαία διαφορά χωρίς ημερήσια υπέρβαση της τεκμαρτής βάσης, εφαρμόζεται ο εφεδρικός κανόνας από την τελευταία εργάσιμη ημέρα προς τις προηγούμενες.
- Ο κανόνας 6ης ημέρας εφαρμόζεται μόνο σε πλήρη απασχόληση: με 7 αναγνωρισμένες ημέρες επιλέγει έως 2 μικρότερες βάσεις, με 6 ημέρες και σύνολο άνω των 40 ωρών έως 1, και σε ισοβαθμία ακολουθεί Κυριακή→Δευτέρα.
- Η παράμετρος καταστήματος `sunday_rest_transfer_enabled` και το πρόγραμμα της επόμενης εβδομάδας εφαρμόζουν την εξαίρεση Κυριακής άνω των 5 ωρών με τουλάχιστον 3 ρητά `ΑΝΑΠΑΥΣΗ/ΡΕΠΟ`. Άγνωστη επόμενη εβδομάδα δεν ακυρώνει τη 6η ημέρα.
- Η 6η ημέρα περιλαμβάνει μόνο την καθαρή βάση και διασπάται σε ημέρα/νύχτα/Κυριακή/νύχτα-Κυριακή· υπερεργασία και υπερωρία παραμένουν αποκλειστικά στις δικές τους κατηγορίες.
- Τεκμηριώθηκε ρητά η ημερήσια βάση υπερωρίας: δηλωμένο ακριβώς `8:00` σημαίνει υπερωρία μετά την 9η ώρα, δηλωμένο ακριβώς `6:40` μετά την 8η ώρα, ενώ κάθε άλλη δηλωμένη διάρκεια ακολουθεί την αντίστοιχη βάση της σύμβασης. Η διάρκεια του χτυπήματος δεν επιλέγει τη βάση και το `break_in_work` δεν πρέπει να μετακινεί λανθασμένα τα όρια.
- Το `docs/APOLOGISTIKO_LOGIC.md` περιλαμβάνει πλέον συγκεντρωμένα όλους τους οριστικούς κανόνες ωρομέτρησης και τα σενάρια πλήρους/μερικής απασχόλησης και 6ης ημέρας.
- Στις εγκεκριμένες Μεταβολές, η Πρόταση αποτελεί πλέον το δηλωμένο ωράριο για την ωρομέτρηση και εμφανίζεται ως τέτοιο στην πλήρη ανάλυση. Το αρχικό δηλωμένο διατηρείται μόνο ως ιστορικό του Απολογιστικού.

---

## 2026-08-20 — AI Agent: ταχύτερο prompt

- Συμπύκνωση οδηγιών σε `guide` (χωρίς διπλότυπα philosophy/rules).
- Το `today_home` χτίζεται/στέλνεται **μόνο** όταν το μήνυμα μοιάζει με ερώτηση σήμερα ή ομαδική εντολή («όσους τελειώνουν…»)· απλό «άνοιξε κάρτα του Χ» το παραλείπει (λιγότερο DB + tokens).

---

## 2026-08-20 — AI Agent: ακύρωση σημασιολογικά (cancel_pending)

- Νέο intent `cancel_pending`: το Gemini κρίνει αν το μήνυμα είναι απόρριψη/ακύρωση (οποιαδήποτε διατύπωση· όχι whitelist).
- Παραδείγματα στο prompt είναι ενδεικτικά («γάμα το», «άστο», «ακύρωσε την #N» κλπ.), όχι δεσμευτικά.
- Αφαίρεση άκαμπτου Python matcher φράσεων ακύρωσης.

---

## 2026-08-20 — AI Agent: διευκρινίσεις ονομάτων μέσω Gemini (όχι Python stems)

- Απαντήσεις τύπου «διονυση» / `dionisi` στέλνονται **ως έχουν** στο Gemini.
- Νέο `philosophy` + κανόνες στο prompt: φωνητική αντιστοίχιση, greeklish, name_choice context, απαγόρευση meta-συγγνώμης («το διορθώνω»).
- Το Python κρατά μόνο safety-net ασάφειας επωνύμου στην αρχική εντολή· μετά από `name_choice` δεν ξαναρωτά.

---

## 2026-08-20 — AI Agent: απάντηση «Διονυσίου» σε Εννοείτε…

- Η απάντηση σε ερώτηση «Εννοείτε του Χ ή του Υ;» δένει στο **ίδιο** `#N` και επιλέγει τον εργαζόμενο (γενική πτώση / stem ονόματος), χωρίς νέο γύρο ασάφειας.
- Δεν ξαναπερνάει το κείμενο της ερώτησης (που περιέχει όλα τα ονόματα) στο matcher — αυτό ήταν που προκαλούσε το loop.
- Αποθηκεύονται `ambiguous_employee_afms` στο payload· παλιές εντολές ανακτούν υποψηφίους από το κείμενο της ερώτησης.

---

## 2026-08-20 — AI Agent: ασάφεια παρόμοιων ονομάτων

- Όταν υπάρχουν ίδια ή πολύ παρόμοια επώνυμα (π.χ. ΦΩΤΟΠΟΥΛΟΣ/ΦΩΤΟΠΟΥΛΟΥ, δύο VLASENKO), δεν μαντεύει· ζητά διευκρίνιση.
- Η διευκρίνιση απαριθμεί **όλες** τις επιλογές με άρθρο φύλου, π.χ. «Εννοείτε του Κωνσταντινου, του Διονυσιου ή της Ελενης;».
- Αν δοθεί μοναδικό όνομα (π.χ. «της Ελένης»), προχωρά κανονικά χωρίς διευκρίνιση.

---

## 2026-08-20 — AI Agent: απάντηση σε διευκρίνιση δεν είναι νέα εντολή

- Αν υπάρχει ανοιχτή εντολή σε `needs_clarification`, το επόμενο μήνυμα του χρήστη **δένει σε αυτή** (chat και Telegram).
- «Ναι» / «ok» σε ερώτηση ακύρωσης → ακυρώνει την ίδια εντολή (`cancelled`), χωρίς νέο `#N`.
- Άλλες απαντήσεις επανυπολογίζουν την **ίδια** εντολή αντί να ανοίγουν νέα.

---

## 2026-08-19 — Απολογιστικός: overnight carry μόνο από ώρες (χωρίς `*`)

- Ο κανόnas συνέχειας βραδυνής εργασίας (`_partition_punches_covered_by_previous_overnight`) κρίνει **μόνο από τις ώρες**: μία θεωρούμενη έναρξη, παράθυρο 13ω/12ω + διάλειμμα, πρόσθετα ζεύγη επόμενης ημέρας πριν την κύρια βάρδια → συνέχεια προηγούμενης.
- **Αφαιρέθηκε** η απαίτηση ρητής λήξης με `*` και ο έλεγχος «μετά τη λήξη *».
- Νέα έκδοση υπολογισμού: `2026-08-19.rules-v10-overnight-by-times`.
- Επανυπολογισμός αποθηκευμένου απολογιστικού **1/6 – 18/8/2026** (144 runs, 12 εβδομάδες × 12 καταστήματα, όλα OK).

---

## 2026-08-19 — Ωρομέτρηση απολογιστικού (εβδομάδα + μήνας)

- Νέος καθαρός υπολογιστικός πυρήνας `app/timekeeping.py` για αναγνωρισμένο ωράριο βάσης, συνεχόμενο διάλειμμα, νύχτα, Κυριακή/αργία, πρόσθετη εργασία μερικής, 6η ημέρα και ζώνες υπερωρίας 40%/60%/120%.
- Το ετήσιο υπόλοιπο υπερωρίας φορτώνεται μαζικά από το `karta_portal_schedule_archive` έως την προηγούμενη ημέρα της εβδομάδας, χωρίς query ανά εργαζόμενο (`CAST(synced_at AS datetime2)` για συμβατότητα pyodbc).
- Στο `/ui/apologistic` εμφανίζεται κουμπί **Ωρομέτρηση** / **Ωρομέτρηση μήνα** όταν δεν υπάρχουν εγγραφές Ελέγχου (εβδομαδιαία ή μηνιαία προβολή).
- **Ωρομέτρηση μήνα**: ενώνει εβδομαδιαίους υπολογισμούς με σωστό running annual overtime context· preview και Excel export (`year`/`month`).
- Όταν λείπουν εγκρίσεις, η σελίδα ωρομέτρησης μήνα εμφανίζει τις **προβληματικές εβδομάδες** ως clickable links προς το απολογιστικό με φίλτρο «Για έλεγχο».
- Το «← Απολογιστικό» θυμάται mode/εβδομάδα/μήνα/φίλτρο/ημέρα (URL `origin_*` params).
- Οι ημερομηνίες εμφανίζονται ως `ηη/μμ/εεεε` και οι κατηγορίες ως **Κυρ/Αργία** και **6η ημέρα**.
- Η σελίδα ωρομέτρησης προσφέρει εξαγωγή Excel με φύλλα **Σύνοψη** και **Ανά ημέρα**, αριθμητικές διάρκειες `[h]:mm`, φίλτρα και παγωμένες κεφαλίδες.

---

## 2026-08-19 — Απολογιστικός: μαζικές ενέργειες και διατήρηση προβολής

- Νέο κουμπί **Επαναφορά αλλαγών** — επαναφέρει όλες τις εγγραφές Μ* (από Έλεγχο) στην αρχική κατάσταση Ε.
- Μετά **Έγκριση όλων** (Ε→Μ*): αυτόματη εναλλαγή φίλτρου σε «Μεταβολές».
- Μετά **Επαναφορά αλλαγών** (Μ*→Ε): αυτόματη εναλλαγή φίλτρου σε «Για έλεγχο».
- Κατά αλλαγή μήνα/εβδομάδας μηδενίζονται αμέσως οι μετρητές και τα week buttons πριν φορτωθούν τα νέα δεδομένα.
- Το φίλτρο και η επιλεγμένη ημέρα διατηρούνται κατά φόρτωση δεδομένων και επιστροφή από ωρομέτρηση (URL query params).

---

## 2026-08-19 — AI Agent: φιλτράρισμα εργαζομένων ανά κατάστημα + Αργίες

- **AI Agent**: Όταν το κατάστημα είναι γνωστό (reply, focus, αναφορά), στέλνεται στο Gemini μόνο η λίστα εργαζομένων και η σημερινή εικόνα **αυτού** του καταστήματος — αποτρέπει hallucinations σε λάθος ΑΦΜ.
- **Αργίες**: Νέοι πίνακες `karta_holidays` (γενικές αργίες Ελλάδας) & `karta_store_holiday_overrides` (ανά κατάστημα). Νέο module `app/repo_holidays.py` με `is_holiday_or_sunday()` αντί `is_sunday`. UI διαχείρισης στο `ui/stores/notify`.
- **Απολογιστικός**: Η αντισταθμιστική ανάπαυση υπολογίζεται πλέον και για επίσημες αργίες (όχι μόνο Κυριακές).
