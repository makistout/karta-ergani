...
| Προβολή | `GET /api/store/active` → `GET /api/schedule/list?date=` ή `?from=&to=` από `karta_schedule` |
| **Συγχρονισμός Ergani** | `POST /api/schedule/sync` body `{ date }` ή `{ from, to }` → `fetch_and_save_schedule_for_ctx` → portal parse ανά ημέρα → `replace_schedule_for_day` |
| Μετά sync | ξανά `loadSchedule()` — πίνακας ΑΦΜ, ημερομηνία, ώρες, βάρδια, διάλειμμα |
...
| 5 | Ταξινόμηση: `_STATUS_ORDER` (πρώτα `at_work`, στο τέλος `rest`) |
...
#### 3. Αναζήτηση ανά ημέρα (`_search_day`)

Για κάθε ημερομηνία `dd/mm/yyyy` στο διάστημα sync (έως 31 ημέρες, `iso_to_ergani_dates`):

1. **Reload** της σελίδας αναζήτησης (`GET page_url`) — καθαρή φόρμα χωρίς προηγούμενο grid.
2. `_find_search_form`: φόρμα με controls `ErgazomenosWorkingSearchControl` ή `DailyWorkTimesSearchControl`.
3. Συμπλήρωση POST:
   - **Παράρτημα:** `PararthmaListEdit` = `_pick_pararthma(html, branch_aa)` — τιμή `<option>` που ταιριάζει στο `branch_aa` του καταστήματος.
   - **Φίλτρα κενά:** ΑΦΜ, επώνυμο, όνομα (όλοι οι εργαζόμενοι του παραρτήματος).
   - **Ημερομηνία:** `DateFromEdit` = `DateToEdit` = η ίδια ημέρα.
   - **Κουμπί:** `SearchControlSearchButton` = `Αναζήτηση`.
4. `POST` στη `action` της φόρμας · αν redirect σε `error.aspx` → σφάλμα.
